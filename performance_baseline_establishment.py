...
from tests.benchmarks.test_pymdp_benchmark import PyMDPBenchmarkSuite, BenchmarkConfig, BenchmarkResult
from tests.benchmarks.test_memory_profiling import MemoryTracker, AgentMemoryValidator, MemoryReport
...
class BaselineReport:
    ...
    measurement_stability: Dict[str, float] = field(default_factory=dict)  # Coefficient of variation
...
class UnifiedMetricsCollector:
    ...
    def __init__(self):
        ...
        self.thresholds = {
            'agent_spawn_p95_ms': PerformanceThreshold(
                'agent_spawn_p95_ms', 50.0, 'ms', 'critical',
                'Agent spawn delays block user workflows', 'CLAUDE.md'
            ),
            'belief_update_p95_ms': PerformanceThreshold(
                'belief_update_p95_ms', 10.0, 'ms', 'high',
                'Slow belief updates reduce agent responsiveness', 'CLAUDE.md'  
            ),
            'memory_per_agent_mb': PerformanceThreshold(
                'memory_per_agent_mb', 34.5, 'MB', 'critical',
                'Memory budget violations cause OOM failures', 'CLAUDE.md'
            ),
            'p95_api_latency_ms': PerformanceThreshold(
                'p95_api_latency_ms', 200.0, 'ms', 'high',
                'API latency affects user experience', 'CLAUDE.md'
            ),
            'ui_render_time_ms': PerformanceThreshold(
                'ui_render_time_ms', 150.0, 'ms', 'medium',
                'UI render delays affect perceived performance', 'CLAUDE.md'
            ),
        }
    ...
    async def collect_unified_metrics(
        ...
        logger.info(f"Starting unified metrics collection (duration: {test_duration_minutes}min, chaos: {include_chaos})")
        ...
        try:
            ...
            if isinstance(pymdp_results, dict):
                ...
                metrics.inference_throughput_ops_per_sec = pymdp_results.get('throughput_ops_per_sec', 0.0)
            ...
            if isinstance(db_results, dict):
                ...
                metrics.db_transaction_conflict_rate = db_results.get('transaction_conflict_rate', 0.0)
            ...
            if isinstance(ws_results, dict):
                ...
                metrics.ws_ui_responsiveness_violations = ws_results.get('ui_responsiveness_violations', 0)
    ...
    async def _collect_pymdp_metrics(self) -> Dict[str, float]:
        ...
        try:
            ...
            return {
                'agent_spawn_p95_ms': spawn_result.timing.p95_ms if spawn_result.success else float('inf'),
                'belief_update_p95_ms': belief_result.timing.p95_ms if belief_result.success else float('inf'),
                'policy_selection_p95_ms': policy_result.timing.p95_ms if policy_result.success else float('inf'),
                'throughput_ops_per_sec': spawn_result.timing.operations_per_second if spawn_result.success else 0.0,
            }
    ...
    async def _collect_websocket_metrics(self, duration_minutes: int, include_chaos: bool) -> RealisticPerformanceResult:
    ...
    async def _monitor_memory_continuously(self, duration_minutes: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(memory_samples) > 10:
                ...
                leak_detected = (late_avg - early_avg) > 10.0  # 10MB increase indicates potential leak
    ...
    def _detect_threshold_violations(self, metrics: UnifiedMetrics) -> List[str]:
        ...
        if metrics.ws_p95_message_latency_ms > self.thresholds['p95_api_latency_ms'].threshold_value:
...
class BaselineEstablisher:
    ...
    async def establish_comprehensive_baseline(
        ...
        logger.info(f"Establishing baseline {baseline_version} with {statistical_runs} statistical runs")
        ...
        for run in range(statistical_runs):
            ...
            metrics = await self.metrics_collector.collect_unified_metrics(
                ...
                include_chaos=include_chaos_testing
            )
        ...
        report.performance_optimization_opportunities = self._identify_optimization_opportunities(baseline_metrics)
        report.resource_scaling_recommendations = self._generate_scaling_recommendations(baseline_metrics)  
        report.architecture_improvement_suggestions = self._suggest_architecture_improvements(baseline_metrics)
        
        # Production readiness assessment
        report.production_readiness_score = self._calculate_production_readiness_score(baseline_metrics)
        report.deployment_risk_assessment = self._assess_deployment_risk(baseline_metrics)
        report.performance_monitoring_recommendations = self._generate_monitoring_recommendations(baseline_metrics)
        
        # Store in history for trend analysis
        self.baseline_history.append(report)
        
        logger.info(f"Baseline {baseline_version} established - Production Readiness: {report.production_readiness_score:.1f}/100")
    ...
    def _calculate_statistical_baseline(self, metric_runs: List[UnifiedMetrics]) -> UnifiedMetrics:
        ...
        baseline = UnifiedMetrics(
            ...
            policy_selection_p95_ms=statistics.median([m.policy_selection_p95_ms for m in metric_runs]),
            inference_throughput_ops_per_sec=statistics.median([m.inference_throughput_ops_per_sec for m in metric_runs]),
            ...
            memory_budget_violations=int(statistics.median([m.memory_budget_violations for m in metric_runs])),
            memory_leak_detected=any(m.memory_leak_detected for m in metric_runs),
            
            # Database Metrics
            db_connection_success_rate=statistics.median([m.db_connection_success_rate for m in metric_runs]),
            db_p95_query_latency_ms=statistics.median([m.db_p95_query_latency_ms for m in metric_runs]),
            db_transaction_conflict_rate=statistics.median([m.db_transaction_conflict_rate for m in metric_runs]),
            db_slow_queries_count=int(statistics.median([m.db_slow_queries_count for m in metric_runs])),
            
            # WebSocket Metrics
            ws_connection_success_rate=statistics.median([m.ws_connection_success_rate for m in metric_runs]),
            ws_p95_message_latency_ms=statistics.median([m.ws_p95_message_latency_ms for m in metric_runs]),
            ws_message_loss_rate=statistics.median([m.ws_message_loss_rate for m in metric_runs]),
            ws_ui_responsiveness_violations=int(statistics.median([m.ws_ui_responsiveness_violations for m in metric_runs])),
            
            # Business Metrics (median)
            user_experience_score=statistics.median([m.user_experience_score for m in metric_runs]),
            system_reliability_score=statistics.median([m.system_reliability_score for m in metric_runs]),
            performance_cost_efficiency=statistics.median([m.performance_cost_efficiency for m in metric_runs]),
            ...
            critical_threshold_violations=list(set().union(*[m.critical_threshold_violations for m in metric_runs])),
        )
        
        return baseline
    
    def _calculate_confidence_intervals(self, metric_runs: List[UnifiedMetrics]) -> Dict[str, Tuple[float, float]]:
        ...
        def calculate_ci(values: List[float], confidence: float = 0.95) -> Tuple[float, float]:
            ...
            return (sorted_values[lower_index], sorted_values[min(upper_index, len(sorted_values) - 1)])
        
        # Key performance metrics
        intervals['agent_spawn_p95_ms'] = calculate_ci([m.agent_spawn_p95_ms for m in metric_runs])
        intervals['belief_update_p95_ms'] = calculate_ci([m.belief_update_p95_ms for m in metric_runs])
        intervals['memory_per_agent_mb'] = calculate_ci([m.memory_per_agent_mb for m in metric_runs])
        intervals['ws_p95_message_latency_ms'] = calculate_ci([m.ws_p95_message_latency_ms for m in metric_runs])
        intervals['user_experience_score'] = calculate_ci([m.user_experience_score for m in metric_runs])
        
        return intervals
    
    def _calculate_measurement_stability(self, metric_runs: List[UnifiedMetrics]) -> Dict[str, float]:
        ...
        stability['belief_update_p95_ms'] = calculate_cv([m.belief_update_p95_ms for m in metric_runs])
        stability['memory_per_agent_mb'] = calculate_cv([m.memory_per_agent_mb for m in metric_runs])
        stability['ws_p95_message_latency_ms'] = calculate_cv([m.ws_p95_message_latency_ms for m in metric_runs])
        
        return stability
    
    def _detect_natural_boundaries(self, metric_runs: List[UnifiedMetrics]) -> Dict[str, Tuple[float, float]]:
        ...
        boundaries['agent_spawn_p95_ms'] = find_boundaries([m.agent_spawn_p95_ms for m in metric_runs])
        boundaries['belief_update_p95_ms'] = find_boundaries([m.belief_update_p95_ms for m in metric_runs])
        boundaries['memory_per_agent_mb'] = find_boundaries([m.memory_per_agent_mb for m in metric_runs])  
        boundaries['ws_p95_message_latency_ms'] = find_boundaries([m.ws_p95_message_latency_ms for m in metric_runs])
    ...
    def _generate_adaptive_thresholds(self, metric_runs: List[UnifiedMetrics]) -> List[Dict[str, Any]]:
    ...
    def _assess_business_impact(self, metrics: UnifiedMetrics) -> Dict[str, Any]:
        ...
        return {
            ...
            'scalability_assessment': 'good' if metrics.system_reliability_score > 85 else 'needs_improvement',
            'cost_optimization_potential': 'high' if metrics.performance_cost_efficiency < 70 else 'low',
    ...
    def _analyze_cost_efficiency(self, metrics: UnifiedMetrics) -> Dict[str, Any]:
        ...
        return {
            ...
            'scaling_cost_projection': 'linear' if metrics.performance_cost_efficiency > 70 else 'exponential',
            'optimization_roi_potential': 'high' if metrics.performance_cost_efficiency < 60 else 'low',
        }
...
async def run_developer_baseline_establishment():
    ...
    print(f"  API Latency <200ms:      {'✅' if metrics.ws_p95_message_latency_ms < 200.0 else '⚠️'}")
