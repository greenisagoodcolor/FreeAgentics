...
class PyMDPAgentBuilder(IAgentBuilder):
    ...
    def _should_create_pymdp_agent(self, specification: AgentSpecification) -> bool:
        ...
        if specification.source_prompt:
            ai_keywords = [
                "active inference", "pymdp", "belief", "uncertainty", 
                "exploration", "curiosity", "free energy", "prediction",
                "environment", "state", "observation", "action", "policy"
            ]
    ...
    async def _build_pymdp_agent(self, specification: AgentSpecification) -> Agent:
        ...
        try:
            ...
            for session in get_db():
                ...
                logger.info(
                    f"Created PyMDP agent {agent.id} ({agent.name}) with GMN specification"
                )
                return agent
    ...
    async def _generate_gmn_specification(self, specification: AgentSpecification) -> Optional[Dict[str, Any]]:
...
Generate only the GMN specification, no additional text."""
    ...
    async def create_runtime_agent(self, agent_record: Agent) -> Optional[BasicExplorerAgent]:
        ...
        try:
            ...
            if (not agent_record.parameters or 
                agent_record.parameters.get("agent_type") != "pymdp_active_inference"):
                return None
