...
class InferenceEngine:
    ...
    def __init__(self, max_workers: int = 4, default_timeout_ms: int = 5000):
        ...
        logger.info(f"InferenceEngine initialized with {max_workers} workers, {default_timeout_ms}ms default timeout")
        ...
        try:
            ...
            if planning_horizon is not None:
                ...
                if hasattr(agent, 'policies'):
                    metadata["num_policies"] = len(agent.policies) if agent.policies is not None else 0
            ...
            if ("invalid observation" in error_msg or 
                "out of bounds" in error_msg or 
                "index" in error_msg and "out of bounds" in error_msg):
            ...
            if qs_current is not None:
                if isinstance(qs_current, list):
                    ...
                    if len(qs_current) == 1 and isinstance(qs_current[0], np.ndarray):
                        ...
                        # Multiple factors
                        beliefs["states"] = [q.tolist() if hasattr(q, 'tolist') else q for q in qs_current]
    ...
    def _update_metrics(self, inference_time_ms: float, success: bool) -> None:
        ...
        if success and total_completed <= 1:
            ...
            alpha = 0.1
            self._metrics["avg_inference_time_ms"] = (alpha * inference_time_ms) + ((1 - alpha) * current_avg)
