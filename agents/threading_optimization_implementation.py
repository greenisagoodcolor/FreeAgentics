...
class WorkStealingThreadPool:
    ...
    def _start_threads(self):
        ...
        for i in range(self.num_threads):
            thread = threading.Thread(target=self._worker, args=(i,), name=f"WorkerThread-{i}")
            thread.start()
...
class AsyncAgentManagerOptimized:
    ...
    def start(self):
        ...
        self._loop_thread = threading.Thread(target=self._run_event_loop, name="AsyncEventLoop")
    ...
    def step_agent_optimized(self, agent_id: str, observation: Any):
        ...
        asyncio.run_coroutine_threadsafe(
            self.save_agent_state_async(agent_id, {"last_action": result}), self._loop
        )
...
def benchmark_optimizations():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=16) as pool:
        futures = []
        for i in range(1000):
            futures.append(pool.submit(lockfree_registry.register, f"agent_{i}", f"data_{i}"))
        for f in futures:
            f.result()
