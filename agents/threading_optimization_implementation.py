    ...
    def _maybe_resize(self):
        ...
        if target_size != self._current_workers:
            with self._resize_lock:
                ...
                logger.info(f"Resizing thread pool from {self._current_workers} to {target_size} for {workload_type} workload")
...
class WorkStealingThreadPool:
    ...
    def submit(self, task: Callable):
        ...
        for i, (queue, lock) in enumerate(zip(self.work_queues, self.queue_locks)):
            with lock:
                size = len(queue)
                if size < min_size:
                    min_size = size
