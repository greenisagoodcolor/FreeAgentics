...
class EpistemicValueCalculationEngine:
    ...
    def calculate_epistemic_value(
        ...
        knowledge_entropy = self._calculate_knowledge_entropy(
            belief_distribution)
        ...
        if prior_distribution is not None:
            ...
            uniform_prior = np.ones_like(
                belief_distribution) / len(belief_distribution)
            information_gain = self._calculate_information_gain(
                belief_distribution, uniform_prior)
        ...
        certainty_measure = self._calculate_certainty_measure(
            belief_distribution)
        ...
        epistemic_state = EpistemicState(
            ...
            information_sources=[agent_id],
            # Will be updated with network interactions
        )
        ...
        logger.debug(
            f"Calculated epistemic value for agent {agent_id}: {
                epistemic_value:.4f}")
    ...
    def calculate_knowledge_propagation(
        ...
        information_transferred = self._calculate_information_transfer(
            source_state.belief_distribution,
            target_state.belief_distribution,
            shared_information)
        ...
        target_state.confidence_level = self._calculate_confidence_level(
            updated_beliefs)
    ...
    def calculate_collective_intelligence_metrics(
        # agent_id -> list of connected agents
        self, agent_network: Dict[str, List[str]]
        ...
        knowledge_distribution = self._calculate_knowledge_distribution_efficiency(
            agent_network)
        ...
        logger.info(
            f"Calculated collective intelligence metrics: " f"entropy={
                network_entropy:.4f}, diversity={
                information_diversity:.4f}, " f"consensus={
                consensus_level:.4f}")
    ...
    def _calculate_knowledge_entropy(
            self, belief_distribution: np.ndarray) -> float:
    ...
    def _calculate_confidence_level(
        ...
        current_entropy = self._calculate_knowledge_entropy(
            belief_distribution)
    ...
    def _calculate_certainty_measure(
            self, belief_distribution: np.ndarray) -> float:
        """Calculate certainty as concentration of probability mass"""
        # Normalize distribution to ensure it sums to 1
        normalized_beliefs = (
            belief_distribution / np.sum(belief_distribution)
            if np.sum(belief_distribution) > 0
            else belief_distribution
        )

        # Use concentration measure based on maximum probability
        max_prob = np.max(normalized_beliefs)
        uniform_prob = 1.0 / len(
            normalized_beliefs
        )  # What each element would be in uniform distribution

        # Certainty ranges from 0 (uniform) to 1 (completely concentrated)
        certainty = (max_prob - uniform_prob) / \
            (1.0 - uniform_prob) if uniform_prob < 1.0 else 0.0
    ...
    def _calculate_knowledge_distribution_efficiency(
            self, network: Dict[str, List[str]]) -> float:
    ...
    def _calculate_emergence_indicator(self) -> float:
        ...
        emergence = current_efficiency * \
            len(self.agent_states) / individual_sum
    ...
    def _calculate_network_stability(self) -> float:
        ...
        # Last 5 measurements
        recent_metrics = self.network_metrics_history[-5:]
        consensus_variance = np.var(
            [m.consensus_level for m in recent_metrics])
