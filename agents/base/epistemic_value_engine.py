...
class EpistemicValueCalculationEngine:
    ...
    def _calculate_certainty_measure(self, belief_distribution: np.ndarray) -> float:
        ...
        certainty = (2 * np.sum(index * sorted_beliefs)) / (n * np.sum(sorted_beliefs)) - (
            n + 1
        ) / n
    ...
    def _calculate_overall_epistemic_value(
        ...
        """Calculate overall epistemic value as weighted combination of metrics."""
    ...
    def _calculate_collective_accuracy(self) -> float:
        """Calculate collective accuracy (placeholder for ground truth comparison)."""
