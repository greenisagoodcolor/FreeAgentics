...
class PersonalityTrait:
    ...
    temporary_modifiers: Dict[str, tuple[float, datetime]] = field(
        default_factory=dict)
    evolution_history: List[tuple[datetime, float, str]
    ...
    def add_temporary_modifier(self, name: str, value: float,
    ...
    def evolve_trait(
        ...
        evolution_amount = (experience_impact *
                            self.definition.evolution_rate *
                            (1 -
                             self.definition.stability))
        # Apply evolution with some randomness
        random_factor = random.uniform(-0.1, 0.1)
        total_change = evolution_amount + random_factor
        # Update trait value
        old_value = self.current_value
        self.current_value = self.definition.validate_value(
            self.current_value + total_change)
        # Record evolution history
        if abs(
                old_value -
                self.current_value) > 0.001:  # Only record significant changes
            self.evolution_history.append(
                (datetime.now(), total_change, context))
        ...
        for trait in personality_profile.traits.values():
            ...
            if behavior_type in trait.definition.behavior_modifiers:
                ...
                if trait.definition.influence_type == TraitInfluence.MULTIPLICATIVE:
                    ...
                    trait_modifier = 1.0 + \
                        (trait_value - 0.5) * (base_modifier - 1.0)
...
class PersonalityProfile:
    ...
    def evolve_traits_from_experience(
            self, experience_data: Dict[str, Any]) -> None:
        ...
        if experience_type == "exploration":
            self._evolve_trait_if_exists(
                "openness", base_impact, "exploration_experience")
            self._evolve_trait_if_exists(
                "curiosity", base_impact, "exploration_experience")
        elif experience_type == "social_interaction":
            if social_context:
                self._evolve_trait_if_exists(
                    "extraversion", base_impact, "social_experience")
                self._evolve_trait_if_exists(
                    "agreeableness",
            ...
            self._evolve_trait_if_exists(
                "risk_tolerance", risk_impact, "risk_experience")
            if not success:
                self._evolve_trait_if_exists(
                    "neuroticism", abs(risk_impact), "failed_risk")
        elif experience_type == "cooperation":
            self._evolve_trait_if_exists(
                "agreeableness", base_impact, "cooperation_experience")
            self._evolve_trait_if_exists(
                "empathy", base_impact, "cooperation_experience")
        elif experience_type == "leadership":
            self._evolve_trait_if_exists(
                "dominance", base_impact, "leadership_experience")
            self._evolve_trait_if_exists(
                "extraversion",
                base_impact * 0.5,
    ...
    def get_behavior_modifier(self, behavior_type: str) -> float:
        ...
        return self.personality_system.calculate_behavior_modifier(
            self, behavior_type)

    def get_interaction_effect(self, interaction_type: str) -> float:
        """Get the personality effect on social interactions"""
        return self.personality_system.calculate_interaction_effect(
        ...
        if "traits" in data:
            for trait_name, trait_data in data["traits"].items():
                if trait_name in profile.traits:
                    ...
                    trait.current_value = trait_data.get(
                        "current_value", trait.current_value)
                    trait.base_value = trait_data.get(
                        "base_value", trait.base_value)
...
def create_personality_profile(
    ...
    return _global_personality_system.create_personality_profile(
        agent_type, trait_values)
