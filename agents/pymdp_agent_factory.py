...
class PyMDPAgentFactory:
    ...
    def validate_gmn_spec(self, gmn_spec: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if len(A_matrices) != len(num_obs):
                logger.error(
                    f"Number of A matrices ({len(A_matrices)}) must match number of observation modalities ({len(num_obs)})"
                )
                return False

            if len(B_matrices) != len(num_states):
                logger.error(
                    f"Number of B matrices ({len(B_matrices)}) must match number of state factors ({len(num_states)})"
                )
                return False
    ...
    def _validate_matrices(
        self, A: List[NDArray], B: List[NDArray], C: List[NDArray], D: List[NDArray]
    ) -> None:
        ...
        for i, B_matrix in enumerate(B):
            if B_matrix.ndim != 3:
                raise PyMDPAgentCreationError(
                    f"B matrix {i} must have exactly 3 dimensions (next_state, state, action)"
                )
    ...
    def _update_metrics(self, creation_time_ms: float) -> None:
        ...
        if total_created <= 1:
            ...
            alpha = 0.1
            self._metrics["avg_creation_time_ms"] = (alpha * creation_time_ms) + (
                (1 - alpha) * current_avg
            )
