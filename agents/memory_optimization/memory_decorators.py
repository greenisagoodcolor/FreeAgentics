    ...
    def check_usage(self, current_mb: float) -> Optional[str]:
        ...
        if current_mb > self.limit_mb:
            if self.enforcement_mode == "raise":
                raise MemoryError(
                    f"Memory usage {current_mb:.2f}MB exceeds limit {self.limit_mb:.2f}MB"
                )
...
def configure_memory_profiling(
    enabled: bool = True, sampling_rate: float = 1.0, default_budget_mb: Optional[float] = None
):
...
def profile_memory(
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args, **kwargs):
            ...
            current_sampling = (
                sampling_rate if sampling_rate is not None else _profiling_context.sampling_rate
            )
                ...
                if detailed and snapshot_before and snapshot_after:
                    ...
                    if significant_changes:
                        ...
                        for stat in significant_changes[:5]:
                            logger.info(
                                f"  {stat.traceback.format()[-1].strip()}: {stat.size_diff/1024/1024:+.2f}MB"
                            )
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args, **kwargs):
            ...
            if len(call_history) >= window_size:
                ...
                growth_points = sum(
                    1 for i in range(1, len(call_history)) if call_history[i] > call_history[i - 1]
                )
...
def agent_memory_profile(
    agent_id: Optional[str] = None, budget_mb: float = 34.5
) -> Callable[[F], F]:
