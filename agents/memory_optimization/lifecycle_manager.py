...
class AgentMemoryProfile:
    ...
    def update_memory_usage(
        self, belief_mb: float = 0.0, matrix_mb: float = 0.0, other_mb: float = 0.0
    ):
...
class AgentMemoryLifecycleManager:
    ...
    def register_agent(
        self, agent_id: str, memory_limit_mb: Optional[float] = None, agent_ref: Any = None
    ) -> AgentMemoryProfile:
        ...
        with self._lock:
            ...
            profile.record_lifecycle_event(
                "registered",
                {"memory_limit_mb": memory_limit_mb, "total_agents": len(self._profiles)},
            )
    ...
    def agent_memory_context(self, agent_id: str):
        ...
        try:
            ...
            profile.record_lifecycle_event(
                "memory_context_error", {"error": str(e), "error_type": type(e).__name__}
            )
            raise
        finally:
    ...
    def hibernate_agent(self, agent_id: str) -> bool:
        ...
        with self._lock:
            ...
            if profile.state in [AgentLifecycleState.HIBERNATING, AgentLifecycleState.TERMINATED]:
                return True
    ...
    def cleanup_idle_agents(self) -> Dict[str, int]:
        ...
        cleanup_stats = {"hibernated": 0, "recycled": 0, "memory_freed_mb": 0.0}
        ...
        with self._lock:
            ...
            for agent_id, profile in self._profiles.items():
                ...
                if (
                    ...
                    profile.state in [AgentLifecycleState.IDLE, AgentLifecycleState.HIBERNATING]
...
def update_agent_memory_usage(
    agent_id: str, belief_mb: float = 0.0, matrix_mb: float = 0.0, other_mb: float = 0.0
) -> bool:
    """Update memory usage for an agent."""
