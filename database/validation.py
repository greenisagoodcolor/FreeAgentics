...
def validate_model_data(model_class: Type, data: Dict[str, Any]) -> bool:
    """Validate data for a SQLAlchemy model.
    ...
    Returns:
        True if data is valid for the model, False otherwise
    """
    try:
        # Check if model_class has required attributes
        if not hasattr(model_class, "__tablename__"):
            return False

        # Try to create instance with the data (basic validation)
        # This is a simple validation - just check if we can instantiate
        model_class(**data)
        return True
