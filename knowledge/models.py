...
class KnowledgeNode(Base):
    ...
    edges_from = relationship("KnowledgeEdge", foreign_keys="KnowledgeEdge.source_id", back_populates="source")
    edges_to = relationship("KnowledgeEdge", foreign_keys="KnowledgeEdge.target_id", back_populates="target")
    ...
    if PGVECTOR_AVAILABLE:
        ...
        __table_args__ += (
            Index('idx_kg_nodes_embedding', 'embedding', postgresql_using='ivfflat', 
...
class AgentBeliefSnapshot(Base):
    ...
    if PGVECTOR_AVAILABLE:
        __table_args__ += (
            Index('idx_belief_vector', 'belief_vector', postgresql_using='ivfflat',
                  postgresql_with={'lists': 100}, postgresql_ops={'belief_vector': 'vector_cosine_ops'}),
        )
...
def update_agent_model():
    ...
    if not hasattr(Agent, 'knowledge_nodes'):
        Agent.knowledge_nodes = relationship("KnowledgeNode", back_populates="agent", cascade="all, delete-orphan")
    
    if not hasattr(Agent, 'belief_snapshots'):
        Agent.belief_snapshots = relationship("AgentBeliefSnapshot", back_populates="agent", cascade="all, delete-orphan")