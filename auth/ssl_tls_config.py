...
class TLSConfiguration:
    ...
    tls12_ciphers: List[str] = field(default_factory=lambda: [
    ...
    # Elliptic Curves
    elliptic_curves: List[str] = field(default_factory=lambda: [
        'X25519',
        'secp256r1',
        'secp384r1',
    ])
...
class OCSPStapler:
    ...
    def fetch_ocsp_response(self, cert_path: str) -> Optional[bytes]:
        ...
        try:
            ...
            cmd = [
                'openssl', 'ocsp',
                '-issuer', self.config.ca_cert_file,
                '-cert', cert_path,
                '-url', ocsp_url,
                '-resp_text'
            ]
...
def validate_ssl_configuration(context: ssl.SSLContext) -> Dict[str, bool]:
    ...
    validation_results["tls_1_3_supported"] = hasattr(ssl, 'TLSVersion') and hasattr(ssl.TLSVersion, 'TLSv1_3')
    ...
    validation_results["renegotiation_disabled"] = bool(context.options & ssl.OP_NO_RENEGOTIATION) if hasattr(ssl, 'OP_NO_RENEGOTIATION') else True
