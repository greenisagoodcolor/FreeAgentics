                ...
                results["secrets"] = {
                    ...
                    "files": list(
                        set(
                            [
                                line.split(":")[0]
                                for line in secrets_check.stdout.strip().split("\n")
                            ]
                        )
                    ),
                }
...
"""
    ...
    def run_continuous_monitoring(self, interval_minutes: int = 60):
        ...
        try:
            while True:
                ...
                # Save report
                report_path = Path(
                    f"security_reports/report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
                )
    ...
    def _has_critical_issues(self, scan_results: Dict[str, Any]) -> bool:
        ...
        return (
            scan_results.get("dependencies", {}).get("vulnerable", 0) > 0
            or scan_results.get("secrets", {}).get("found", False)
            or scan_results.get("authentication", {}).get("brute_force_detected", False)
        )
