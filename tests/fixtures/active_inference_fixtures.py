...
def hierarchical_generative_model(model_parameters):
    ...
    class HierarchicalGenerativeModel(GenerativeModel):
        def __init__(self, params: ModelParameters):
            ...
            for dims in self.levels:
                ...
                for a in range(dims.num_actions):
                    level_model['B'][a] = level_model['B'][a] / level_model['B'][a].sum(dim=0, keepdim=True)
        ...
        def compute_top_down_prediction(self, level: int, higher_state: torch.Tensor) -> torch.Tensor:
