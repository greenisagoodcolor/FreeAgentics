...
def main():
    ...
    single_parser.add_argument(
        "--duration", "-d", type=float, help="Override scenario duration (seconds)"
    )
    single_parser.add_argument(
        "--spawn-rate", "-r", type=float, help="User spawn rate (users per second)"
    )
    ...
    single_parser.add_argument(
        "--db-url", help="Database URL (default: postgresql://localhost/freeagentics_test)"
    )
    ...
    schedule_parser.add_argument(
        "schedule", help="Schedule to run ('daily', 'stress', or path to JSON file)"
    )
