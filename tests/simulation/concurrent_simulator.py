...
from tests.websocket_load.client_manager import (
    WebSocketClient,
...
class SimulatedUser:
    ...
    def _get_initial_subscriptions(self) -> List[str]:
        ...
        return persona_subscriptions.get(
            self.behavior.profile.persona_type,
            ["agent:created", "agent:action"],
        )
...
class ConcurrentSimulator:
    ...
    async def _spawn_users(self, count: int):
        ...
        if diff > 0:
            ...
            most_common = max(
                persona_counts.keys(),
                key=lambda p: self.config.user_distribution[p],
            )
    ...
    async def _export_results(self):
        ...
        # Export metrics
        metrics_file = (
            self.config.results_path / f"metrics_{self.config.name}_{timestamp}.json"
        )
        ...
        behaviors_file = (
            self.config.results_path / f"behaviors_{self.config.name}_{timestamp}.json"
        )
        behaviors = {
            user_id: {
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            "config": {
                ...
                "distribution": {
                    p.value: c for p, c in self.config.user_distribution.items()
                },
            },
