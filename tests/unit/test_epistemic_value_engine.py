...
class TestEpistemicValueCalculationEngine:
    ...
    def test_calculate_epistemic_value_with_prior(self, engine, sample_belief_distribution, uniform_belief_distribution, sample_observations):
    ...
    def test_knowledge_entropy_calculation(self, engine, uniform_belief_distribution, concentrated_belief_distribution):
    ...
    def test_information_gain_calculation(self, engine, sample_belief_distribution, uniform_belief_distribution):
        """Test information gain (KL divergence) calculation."""
        # Information gain from uniform to sample distribution
        info_gain = engine._calculate_information_gain(sample_belief_distribution, uniform_belief_distribution)
        ...
        self_gain = engine._calculate_information_gain(sample_belief_distribution, sample_belief_distribution)
        assert abs(self_gain) < 1e-10  # Should be essentially zero

    def test_confidence_level_calculation(self, engine, uniform_belief_distribution, concentrated_belief_distribution):
        """Test confidence level calculation."""
        uniform_confidence = engine._calculate_confidence_level(uniform_belief_distribution)
        concentrated_confidence = engine._calculate_confidence_level(concentrated_belief_distribution)
    ...
    def test_certainty_measure_calculation(self, engine, uniform_belief_distribution, concentrated_belief_distribution):
        """Test certainty measure calculation."""
        uniform_certainty = engine._calculate_certainty_measure(uniform_belief_distribution)
        concentrated_certainty = engine._calculate_certainty_measure(concentrated_belief_distribution)
    ...
    def test_overall_epistemic_value_calculation(self, engine):
        ...
        epistemic_value = engine._calculate_overall_epistemic_value(entropy, info_gain, confidence, certainty)
        
        assert isinstance(epistemic_value, float)
        assert 0.0 <= epistemic_value <= 1.0

    def test_calculate_knowledge_propagation(self, engine, sample_belief_distribution, uniform_belief_distribution, sample_observations):
        ...
        engine.calculate_epistemic_value(target_id, uniform_belief_distribution, sample_observations)
    ...
    def test_belief_update_bayesian(self, engine):
        ...
        updated = engine._update_beliefs_with_shared_info(target_beliefs, shared_info, "bayesian_update")
    ...
    def test_belief_update_weighted_average(self, engine):
        ...
        updated = engine._update_beliefs_with_shared_info(target_beliefs, shared_info, "weighted_average")
    ...
    def test_calculate_collective_intelligence_metrics(self, engine, sample_belief_distribution, uniform_belief_distribution, sample_observations):
    ...
    def test_collective_intelligence_insufficient_agents(self, engine):
        ...
        # One agent
        engine.calculate_epistemic_value("solo_agent", np.array([0.25, 0.25, 0.25, 0.25]), np.array([1, 0, 1, 0]))
        metrics_solo = engine.calculate_collective_intelligence_metrics({"solo_agent": []})
        
        assert metrics_solo.information_diversity == 0.0  # Can't calculate diversity with one agent
