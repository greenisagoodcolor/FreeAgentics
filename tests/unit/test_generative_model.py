...
class TestGenerativeModel:
    ...
    def test_device_selection(self, model_dimensions, model_parameters):
        ...
        class ConcreteModel(GenerativeModel):
            def observation_model(self, states): return torch.zeros(3)
            def transition_model(self, states, actions): return torch.zeros(4)
            def get_preferences(self, timestep=None): return torch.zeros(3)
            def get_initial_prior(self): return torch.ones(4) / 4
...
class TestDiscreteGenerativeModel:
    ...
    def test_initialization(self, model_dimensions, model_parameters):
        ...
        assert model.B.shape == (model_dimensions.num_states, model_dimensions.num_states, model_dimensions.num_actions)
...
class TestHierarchicalGenerativeModel:
    ...
    def test_hierarchical_state_handling(self, model_parameters):
        ...
        for level in range(model.num_levels):
            level_state = torch.ones(model.dimensions[level].num_states) / model.dimensions[level].num_states
...
class TestCreateGenerativeModel:
    ...
    def test_create_discrete_model(self, model_dimensions, model_parameters):
        ...
        model = create_generative_model("discrete", dimensions=model_dimensions, parameters=model_parameters)
    ...
    def test_create_continuous_model(self, model_dimensions, model_parameters):
        ...
        model = create_generative_model("continuous", dimensions=model_dimensions, parameters=model_parameters)
    ...
    def test_invalid_model_type(self, model_dimensions, model_parameters):
        ...
        with pytest.raises(ValueError):
            create_generative_model("invalid_type", dimensions=model_dimensions, parameters=model_parameters)
