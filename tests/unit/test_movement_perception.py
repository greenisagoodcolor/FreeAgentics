...
class TestMovementPerceptionSystem:
    ...
    def test_find_path_astar_nonexistent_start(self, mock_grid_distance):
        ...
        self.mock_world.get_cell.side_effect = lambda x: (
            None if x == "nonexistent" else self.center_cell
        )
    ...
    def test_find_path_astar_nonexistent_goal(self, mock_grid_distance):
        ...
        self.mock_world.get_cell.side_effect = lambda x: (
            self.center_cell if x == self.center_hex else None
        )
    ...
    def test_perception_with_multiple_agents(self):
        ...
        other_agents = [
            ...
            },
            {
                ...
                "position": "far_away",
                "class": "trader",
                "current_action": "trading",
            },
        ]

        visible_cells = [
            self.cells["872830828ffffff"],
            self.cells["87283082effffff"],
            self.cells["87283082affffff"],
        ]
    ...
    def test_exploration_target_selection_strategy(self, mock_distance):
        ...
        for i in range(5):
            cell = HexCell(
                ...
                moisture=50.0 - i * 5,
                # Decreasing resources with distance
                resources={"minerals": 20.0 - i * 3},
            )
        ...
        targets = self.system.get_exploration_targets(
            "872830828ffffff", explored_hexes, num_targets=3
        )

        # Should prioritize closer targets with more resources and biome
        # variety
        assert len(targets) <= 3

        # explore_0 should be highly ranked (close, high resources, different
        # biome)
