...
class TestPenetrationTestingScenarios:
    ...
    def test_brute_force_protection(self, client, auth_manager):
        ...
        auth_manager.register_user(
            "brute_user", "brute@example.com", "secure_password", UserRole.OBSERVER
        )
        ...
        for i in range(max_attempts):
            response = client.post(
                "/auth/login", json={"username": "brute_user", "password": f"wrong_password_{i}"}
            )
    ...
    def test_password_complexity_enforcement(self, auth_manager):
        ...
        for i, weak_password in enumerate(weak_passwords):
            try:
                auth_manager.register_user(
                    f"weak_user_{i}", f"weak{i}@example.com", weak_password, UserRole.OBSERVER
                )
    ...
    def test_session_management_security(self, auth_manager):
        ...
        auth_manager.register_user(
            "session_user", "session@example.com", "session_password", UserRole.OBSERVER
        )
