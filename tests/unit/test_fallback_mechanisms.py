...
class TestIntegrationScenarios:
    ...
    def test_high_resource_scenario(self):
        ...
        assert response is not None
        # Should use better fallback levels, but may still use any level based on implementation
        assert response.fallback_level in [
            FallbackLevel.CACHED,
            FallbackLevel.TEMPLATE,
            FallbackLevel.PRECOMPUTED,
    ...
    def test_performance_monitoring(self):
        ...
        # Some responses may be cached, so actual response count may be less than 5
