...
class TestRBACSecurityAudit:
    ...
    def mock_request(self):
        ...
        request.headers = {"user-agent": "test-agent", "x-forwarded-for": "10.0.0.1, 192.168.1.100"}
        return request
    ...
    async def test_api_endpoint_protection(self, test_users, mock_db, mock_request):
        ...
        async def modify_agent_endpoint(
            agent_id: str, current_user: TokenData, db: Session, request: Request
        ):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await modify_agent_endpoint(
                agent_id=agent_id, current_user=observer, db=mock_db, request=mock_request
            )
    ...
    async def test_department_isolation(self, test_users, mock_request):
        ...
        result = await access_department_resource(
            department="Research", current_user=researcher, request=mock_request
        )
        ...
        with pytest.raises(HTTPException) as exc_info:
            await access_department_resource(
                department="Finance", current_user=researcher, request=mock_request
            )
        assert exc_info.value.status_code == 403
