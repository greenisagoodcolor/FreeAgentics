...
from auth.security_implementation import (
    UserRole, Permission, ROLE_PERMISSIONS, TokenData, auth_manager,
    SecurityValidator
)
from auth.rbac_enhancements import (
    enhanced_rbac_manager, AccessContext, ResourceContext,
    ABACRule, ABACEffect, UserRole as ABACUserRole,
    RoleAssignmentRequest, RequestStatus
)
from auth.resource_access_control import (
    ResourceAccessValidator, require_resource_access, 
    require_ownership, require_department_access
)
...
class TestRBACSecurityAudit:
        ...
        for role, perms in ROLE_PERMISSIONS.items():
            for perm in perms:
                if perm in all_permissions and role != UserRole.ADMIN:
                    ...
                    assert perm in [Permission.VIEW_AGENTS, Permission.VIEW_METRICS], \
    ...
    def test_role_manipulation_vulnerabilities(self, test_users):
        ...
        assert shared_perms == {Permission.VIEW_AGENTS, Permission.VIEW_METRICS}, \
    ...
    def test_injection_attack_prevention(self, test_users, mock_db, mock_request):
        ...
        try:
            result = ResourceAccessValidator.validate_agent_access(
                malicious_user, "'; DROP TABLE agents;--", "view", mock_db, mock_request
            )
    ...
    def test_audit_log_integrity(self):
        ...
        if original_log_size > 0:
            ...
            assert enhanced_rbac_manager.access_audit_log[0].get("tampered") == True
