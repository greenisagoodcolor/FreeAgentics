...
class TestVariationalMessagePassing:
    ...
    def test_discrete_observation_inference(
        ...
        assert beliefs[0] > beliefs[1]  # Should favor state 0 for observation 0
    ...
    def test_convergence(self, inference_config, simple_generative_model):
        ...
        # Run again with same observation - posterior should shift further toward observed state
        ...
        assert beliefs2[0] >= beliefs[0]  # State 0 should have higher probability
...
class TestBeliefPropagation:
    ...
    def test_loopy_belief_propagation(self, inference_config):
        ...
        class CyclicModel(GenerativeModel):
            def __init__(self):
                dims = ModelDimensions(num_states=4, num_observations=4, num_actions=2, time_horizon=5)
...
class TestParticleFilterInference:
    ...
    def test_particle_filtering(self, inference_config, continuous_generative_model):
        ...
        if isinstance(result, tuple):
            ...
            assert mean.shape == (2,)
            # Particles could be 1D (flattened) or 2D (num_particles x state_dim)
...
class TestHierarchicalInference:
    ...
    def test_top_down_bottom_up_integration(self, hierarchical_generative_model):
        ...
        higher_state = torch.ones(hierarchical_generative_model.levels[0].num_states) / hierarchical_generative_model.levels[0].num_states
...
class TestPerformanceOptimization:
    ...
    def test_sparse_computations(self, inference_config):
        ...
        class SparseModel(GenerativeModel):
            def __init__(self):
                dims = ModelDimensions(num_states=100, num_observations=50, num_actions=10, time_horizon=5)
                params = ModelParameters()
                super().__init__(dims, params)
...
class TestIntegration:
    ...
    def test_end_to_end_inference_loop(
        ...
        # Note: convergence behavior depends on the specific observation sequence
        if len(free_energy_trajectory) > 1:
            total_change = sum(abs(free_energy_trajectory[i+1] - free_energy_trajectory[i]) 
...
class TestActiveInferenceEngine:
    ...
    def test_engine_run_inference(self, inference_config, simple_generative_model, sample_observations):
