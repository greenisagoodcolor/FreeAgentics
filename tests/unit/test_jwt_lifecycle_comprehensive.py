...
class TestJWTLifecycle:
    ...
    def test_token_issued_in_future_rejection(self, auth_manager, test_user):
        ...
        assert "not yet valid" in str(exc_info.value).lower() or "iat" in str(exc_info.value).lower()
    ...
    def test_blacklist_cleanup(self, auth_manager, test_user):
        ...
        auth_manager.blacklist[jti] = datetime.now(timezone.utc) - timedelta(days=REFRESH_TOKEN_EXPIRE_DAYS + 2)
