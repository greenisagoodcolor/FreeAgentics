...
This test suite provides complete coverage of the PyMDP-compatible
generative model implementation, including matrix conversions, adapters,
...
# Mock torch before importing the module under test to avoid PyTorch
...
class TestPyMDPGenerativeModel:
    ...
    def test_set_A_matrix_torch(self, pymdp_model, model_dimensions):
        ...
        # Patch torch only within this test
        with patch("inference.engine.pymdp_generative_model.torch", new=MagicMock()):
            # Create mock tensor
            mock_tensor = Mock()
    ...
    def test_set_D_matrix_torch(self, pymdp_model, model_dimensions):
        ...
        with patch("inference.engine.pymdp_generative_model.torch", new=MagicMock()):
            ...
            # Check that it was converted and normalized
            assert isinstance(pymdp_model.D, np.ndarray)
            assert pymdp_model.D.shape == (model_dimensions.num_states,)
            assert np.isclose(pymdp_model.D.sum(), 1.0)
...
class TestPyMDPGenerativeModelAdapter:
    ...
    def test_get_preferences_out_of_bounds(self, discrete_model, model_dimensions):
        ...
        # Get preferences for timestep beyond horizon (should use last
        # timestep)
...
class TestFactoryFunction:
    ...
    def test_create_pymdp_generative_model_basic(self):
        ...
        model = create_pymdp_generative_model(
            num_states=3, num_observations=2, num_actions=2, time_horizon=4
        )
