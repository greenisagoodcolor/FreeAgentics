...
class TestAPIMainSimple:
    ...
    async def test_lifespan_startup_prometheus_success(self):
        ...
        with patch('api.main.logger') as mock_logger:
            with patch('observability.prometheus_metrics.start_prometheus_metrics_collection', mock_prometheus_start):
                with patch('observability.performance_metrics.start_performance_tracking', mock_perf_start):
                    with patch('observability.prometheus_metrics.stop_prometheus_metrics_collection', mock_prometheus_stop):
                        with patch('observability.performance_metrics.stop_performance_tracking', mock_perf_stop):
                            ...
                            mock_logger.info.assert_any_call("✅ Prometheus metrics collection stopped")
    ...
    async def test_lifespan_startup_prometheus_failure(self):
        ...
        with patch('api.main.logger') as mock_logger:
            with patch('observability.prometheus_metrics.start_prometheus_metrics_collection', mock_prometheus_start):
                with patch('observability.performance_metrics.start_performance_tracking', mock_perf_start):
                    with patch('observability.prometheus_metrics.stop_prometheus_metrics_collection', MagicMock()):
                        with patch('observability.performance_metrics.stop_performance_tracking', MagicMock()):
                            async with lifespan(mock_app):
                                pass
                            
                            # Verify failure warning
                            mock_logger.warning.assert_any_call("⚠️ Failed to start Prometheus metrics collection: Prometheus connection failed")
                ...
                with patch('observability.performance_metrics.start_performance_tracking', mock_perf_start):
                    with patch('observability.prometheus_metrics.stop_prometheus_metrics_collection', MagicMock()):
                        with patch('observability.performance_metrics.stop_performance_tracking', MagicMock()):
                            ...
                            # Verify failure warning
                            mock_logger.warning.assert_any_call("⚠️ Failed to start performance tracking: Performance tracking failed")
                ...
                with patch('observability.performance_metrics.start_performance_tracking', mock_perf_start):
                    with patch('observability.prometheus_metrics.stop_prometheus_metrics_collection', mock_prometheus_stop):
                        with patch('observability.performance_metrics.stop_performance_tracking', mock_perf_stop):
                            ...
                            # Verify failure warning
                            mock_logger.warning.assert_any_call("⚠️ Failed to stop Prometheus metrics collection: Prometheus stop failed")
                ...
                with patch('observability.performance_metrics.start_performance_tracking', mock_perf_start):
                    with patch('observability.prometheus_metrics.stop_prometheus_metrics_collection', mock_prometheus_stop):
                        with patch('observability.performance_metrics.stop_performance_tracking', mock_perf_stop):
                            ...
                            # Verify failure warning
                            mock_logger.warning.assert_any_call("⚠️ Failed to stop performance tracking: Performance stop failed")
