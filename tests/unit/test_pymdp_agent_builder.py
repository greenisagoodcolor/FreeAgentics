...
class TestPyMDPAgentBuilder:
    ...
    def mock_personality(self):
        ...
        return PersonalityProfile(
            assertiveness=0.7, analytical_depth=0.9, creativity=0.6, empathy=0.5, skepticism=0.8
        )
    ...
    def test_should_create_pymdp_agent_keyword_detection(self, basic_agent_spec):
        ...
        basic_agent_spec.source_prompt = (
            "Create an agent that uses active inference and belief updating"
        )
        assert builder._should_create_pymdp_agent(basic_agent_spec)

        # Test with PyMDP keywords
        basic_agent_spec.source_prompt = (
            "Agent should explore environment using curiosity and free energy"
        )
        assert builder._should_create_pymdp_agent(basic_agent_spec)
