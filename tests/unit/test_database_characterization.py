...
class TestDatabaseModelsCharacterization:
    ...
    def test_agent_model_structure(self, in_memory_db):
        ...
        agent = Agent(name="Test Agent", template="explorer")
        session.add(agent)
        session.commit()
    ...
    def test_agent_coalition_relationship(self, in_memory_db):
        ...
        coalition = Coalition(name="Test Coalition", goal="Test coordination")
        ...
        result = session.execute(agent_coalition_association.select()).fetchall()
        assert len(result) == 2
    ...
    def test_coalition_model_defaults(self, in_memory_db):
        ...
        # When
        coalition = Coalition(name="Default Test", goal="Testing defaults")
...
class TestDatabaseConnectionManagerCharacterization:
    ...
    def test_connection_pool_configuration(self, mock_create_engine):
        ...
        # When
        DatabaseConnectionManager("postgresql://test/db")
...
class TestDatabaseQueryPatternsCharacterization:
    ...
    def test_bulk_insert_behavior(self, in_memory_db):
        ...
        agents = [Agent(name=f"Bulk Agent {i}", template="test") for i in range(100)]
...
class TestDatabaseErrorHandlingCharacterization:
    ...
    def test_integrity_error_on_duplicate(self, in_memory_db):
        ...
        agent_id = uuid.uuid4()
        agent1 = Agent(id=agent_id, name="First Agent", template="test")
        session.add(agent1)
        session.commit()

        # When - Try to create duplicate ID
        agent2 = Agent(id=agent_id, name="Second Agent", template="test")  # Same ID
        session.add(agent2)
