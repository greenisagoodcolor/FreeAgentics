...
try:
    ...
    class GenerationResponse:
        def __init__(self, text, model, provider, input_tokens=0, output_tokens=0, cost=0.0, latency_ms=0.0, finish_reason=None):
            self.text = text
            self.model = model
...
class TestProviderIntegration:
    ...
    def test_provider_manager_with_multiple_providers(self):
        ...
        with patch('openai.OpenAI'), patch('anthropic.Anthropic'):
            ...
            with patch.object(openai_provider, 'generate') as mock_openai_gen, \
