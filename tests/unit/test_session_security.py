...
class TestSessionCreation:
    ...
    def test_create_multiple_sessions_unique_ids(self):
        ...
        for i in range(100):
            session_id = store.create_session(
                user_id=f"user-{i}", ip_address="192.168.1.1", user_agent="Test"
            )
    ...
    def test_concurrent_session_limit_enforcement(self):
        ...
        for i in range(3):
            session_id = store.create_session(
                user_id=user_id, ip_address=f"192.168.1.{i}", user_agent="Test"
            )
            sessions.append(session_id)

        # Create one more (should remove oldest)
        new_session = store.create_session(
            user_id=user_id, ip_address="192.168.1.99", user_agent="Test"
        )
...
class TestSessionSecurity:
    ...
    def test_get_user_sessions(self):
        ...
        session1 = store.create_session(
            user_id=user_id, ip_address="192.168.1.1", user_agent="Desktop"
        )
        session2 = store.create_session(
            user_id=user_id, ip_address="192.168.1.2", user_agent="Mobile"
        )
