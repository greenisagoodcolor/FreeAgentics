...
from auth.security_implementation import (
    ...
    SecurityValidator, JWT_SECRET_KEY, JWT_ALGORITHM
)
...
from auth.resource_access_control import (
    ...
    require_ownership, _get_required_permission
)
...
class TestAuthorizationPenetration:
    ...
    def test_direct_object_reference_attacks(self, mock_db):
        ...
        sequential_ids = [
            str(uuid.UUID(int=i)) for i in range(1000, 1010)
        ]
    ...
    def test_jwt_token_manipulation(self):
        ...
        none_token = f"{base64.urlsafe_b64encode(json.dumps(header).encode()).decode().rstrip('=')}." \
        ...
        escalated_token = jwt.encode(
            ...
            "wrong-secret-key",  # Using wrong key
