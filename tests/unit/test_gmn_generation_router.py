...
class TestGMNGenerationRouter:
    ...
    def test_generate_gmn_success(
        self, mock_repo_dep, mock_gen_dep, client, mock_generator, mock_repository, 
        sample_gmn_spec, sample_stored_spec
    ):
    ...
    def test_validate_gmn_with_errors(self, mock_gen_dep, client, mock_generator):
        ...
        # Test request
        request_data = {
            "gmn_specification": "invalid gmn spec"
        }
    ...
    @patch("api.v1.routers.gmn_generation.get_gmn_generator")
    @patch("api.v1.routers.gmn_generation.get_gmn_repository")
    def test_generate_gmn_service_error(
        self, mock_repo_dep, mock_gen_dep, client, mock_generator
    ):
    ...
    def test_request_model_validation_comprehensive(self, client):
        ...
        long_prompt = "a" * 2001
        response = client.post("/api/v1/gmn/generate", json={
            "prompt": long_prompt,
            "agent_type": "general"
        })
        assert response.status_code == 422
