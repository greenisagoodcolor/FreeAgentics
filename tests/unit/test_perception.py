...
class TestVisualSensor(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.agent = Agent(
            name="Observer", position=Position(
                0, 0, 0), orientation=Orientation())
    ...
    def test_visual_field_of_view(self) -> None:
        ...
        behind_stimulus = Stimulus(
            "behind", StimulusType.OBJECT, Position(-10, 0, 0))
        ...
        edge_stimulus = Stimulus(
            "edge", StimulusType.OBJECT, Position(
                edge_x, edge_y, 0))
    ...
    def test_visual_range_limit(self) -> None:
        ...
        far_stimulus = Stimulus(
            "far", StimulusType.OBJECT, Position(
                100, 0, 0))
    ...
    def test_visual_confidence_decay(self) -> None:
        ...
        near_percept = next(
            p for p in percepts if p.stimulus.stimulus_id == "near")
        far_percept = next(
            p for p in percepts if p.stimulus.stimulus_id == "far")
...
class TestAuditorySensor(unittest.TestCase):
    ...
    def test_hearing_range(self) -> None:
        ...
        distant_sound = Stimulus(
            "distant", StimulusType.SOUND, Position(
                150, 0, 0), intensity=1.0)
...
class TestPerceptionFilters(unittest.TestCase):
    ...
    def test_importance_filter(self) -> None:
        ...
        self.assertEqual(
            filtered[0].stimulus.stimulus_type,
            StimulusType.DANGER)
    ...
    def test_attention_filter(self) -> None:
        ...
        agent = Agent()
        agent.perception_capabilities = PerceptionCapabilities(
...
class TestPerceptionSystem(unittest.TestCase):
    ...
    def test_agent_registration(self) -> None:
        ...
        self.assertIn(
            ...
            self.perception_system.perception_capabilities)
        self.assertIn(
            self.agent1.agent_id,
            self.perception_system.perception_memories)
    ...
    def test_perceived_agents(self) -> None:
        ...
        self.perception_system.update_agent_positions()
        perceived = self.perception_system.get_perceived_agents(
            self.agent1.agent_id)
    ...
    def test_can_perceive(self) -> None:
        ...
        self.state_manager.update_agent_position(
            self.agent2.agent_id, self.agent2.position)
        self.assertFalse(
            self.perception_system.can_perceive(
                self.agent1.agent_id,
                self.agent2.agent_id,
    ...
    def test_sound_perception(self) -> None:
        ...
        percepts = self.perception_system.perceive(self.agent1.agent_id)
        sound_percepts = [
            p for p in percepts if p.perception_type == PerceptionType.AUDITORY]
