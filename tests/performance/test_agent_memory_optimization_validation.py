...
class AgentMemoryOptimizationValidationTest(unittest.TestCase):
    ...
    def test_single_agent_memory_optimization(self):
        ...
        self.assertLess(optimized_agent_memory, 10.0, 
                       f"Agent memory {optimized_agent_memory:.1f}MB exceeds 10MB target")
        self.assertLess(optimized_agent_memory, initial_agent_memory * 0.5, 
    ...
    def test_multiple_agents_memory_efficiency(self):
        ...
        print(f"Memory efficiency: {(total_agent_memory / (memory_after - self.initial_memory))*100:.1f}%")
        ...
        print(f"  Memory range: {stats['actual_memory_mb']['min']:.1f} - {stats['actual_memory_mb']['max']:.1f} MB")
        
        # Validate requirements
        self.assertEqual(stats['agents_optimized'], num_agents)
        self.assertLess(avg_agent_memory, 10.0, 
                       f"Average agent memory {avg_agent_memory:.1f}MB exceeds 10MB target")
        self.assertLess(stats['actual_memory_mb']['max'], 15.0, 
    ...
    def test_memory_leak_detection(self):
        ...
        if len(memory_snapshots) >= 2:
            ...
            self.assertLess(growth_rate, 2.0, 
    ...
    def test_concurrent_agent_operations(self):
        ...
        self.assertLess(final_memory - initial_memory, 50.0, 
                       "Memory increase during concurrent operations should be <50MB")
        self.assertGreater(total_operations / (end_time - start_time), 100, 
    ...
    def test_memory_optimization_components(self):
        ...
        if optimized._beliefs is not None:
            ...
            compression_ratio = original_beliefs_size / compressed_size if compressed_size > 0 else 1.0
    ...
    def test_system_efficiency_with_50_plus_agents(self):
        ...
        self.assertLess(memory_per_agent_system, 20.0, 
                       f"System memory per agent {memory_per_agent_system:.1f}MB should be <20MB")
        self.assertLess(optimization_time / num_agents, 0.1, 
