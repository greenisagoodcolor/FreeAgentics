...
class CacheComparisonBenchmark(PyMDPBenchmark):
    ...
    def run_iteration(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "efficiency_gain": max(0, (uncached_time - cached_time) / uncached_time)
            * 100,
        }
...
def run_matrix_caching_benchmarks():
    ...
    suite.add_benchmark(
        IntermediateResultCachingBenchmark(complexity_level=2, cache_enabled=True)
    )
    suite.add_benchmark(
        IntermediateResultCachingBenchmark(complexity_level=2, cache_enabled=False)
    )
    suite.add_benchmark(
        IntermediateResultCachingBenchmark(complexity_level=4, cache_enabled=True)
    )
    suite.add_benchmark(
        IntermediateResultCachingBenchmark(complexity_level=4, cache_enabled=False)
    )
