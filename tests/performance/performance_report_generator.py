...
class PerformanceReportGenerator:
    ...
    def detect_regressions(
        ...
        for (benchmark_name, metric_name), metric_list in metric_groups.items():
    ...
    def _create_time_series_chart(
        ...
        for benchmark_name, benchmark_metrics in benchmark_groups.items():
            ...
            plt.plot(timestamps, values, marker="o", label=benchmark_name, linewidth=2)
...
def main():
    ...
    parser.add_argument(
        "--run-all", action="store_true", help="Run all benchmarks before generating reports"
    )
    parser.add_argument(
