...
class IntegratedWebSocketTester:
    ...
    async def test_realistic_agent_conversation_throughput(
        ...
        logger.info(f"Testing agent conversation throughput: {agent_count} agents, {conversation_turns} turns")
                ...
                for agent_id, websocket in agent_connections.items():
                    try:
                        ...
                        kg_msg = {
                            ...
                            "relationships": [{"from": "entity_0", "to": "entity_1", "type": "discusses"}],
                            "confidence": 0.8,
                            "timestamp": time.perf_counter(),
                        }
...
async def test_integrated_agent_conversation_throughput():
    ...
    async with test_server() as server_manager:
        ...
        assert metrics["connections_established"] >= 2, f"Expected at least 2 connections, got {metrics['connections_established']}"
        assert metrics["messages_sent"] >= 15, f"Expected at least 15 messages, got {metrics['messages_sent']}"
        assert metrics["conversations_completed"] >= 2, f"Expected at least 2 conversations, got {metrics['conversations_completed']}"
...
async def test_integrated_connection_stability():
    ...
    async with test_server() as server_manager:
        ...
        if metrics["connection_dropouts"] > 0:
            ...
            assert reconnection_rate >= 0.5, f"Reconnection rate {reconnection_rate:.1f} below 50% threshold"
        ...
        assert metrics["messages_sent"] >= min_expected, f"Expected at least {min_expected} messages, got {metrics['messages_sent']}"
...
async def test_websocket_memory_usage_validation():
    ...
    async with test_server() as server_manager:
        ...
        memory_per_connection_mb = memory_increase_mb / connection_count if connection_count > 0 else 0
        
        # Validate memory usage against 34.5MB per connection budget
        assert memory_per_connection_mb <= 34.5, (
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Run individual tests with: python -m pytest tests/performance/test_websocket_integrated_throughput.py -m integration -v")