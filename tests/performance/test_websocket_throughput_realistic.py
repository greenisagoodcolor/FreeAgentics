...
class WebSocketThroughputTester:
    ...
    async def test_connection_establishment_throughput(
        ...
        establishment_rate_per_second: float = 10.0
    ) -> ThroughputMetrics:
        ...
        success_rate = (metrics.connections_established / target_connections) * 100 if target_connections > 0 else 0
        logger.info(
    ...
    async def test_message_throughput_realistic(
        ...
        throughput_mps = metrics.messages_sent / metrics.test_duration_seconds if metrics.test_duration_seconds > 0 else 0
    ...
    def _generate_realistic_message(self, msg_type: str, connection_id: int, msg_num: int) -> Dict:
        ...
        if msg_type == "heartbeat":
            ...
            base_message.update({
                ...
                "entities": [
                    {"id": f"entity_{i}", "type": "concept", "properties": {"name": f"Entity {i}"}}
    ...
    async def test_multi_agent_coordination_throughput(
        ...
        try:
            ...
            for round_num in range(coordination_rounds):
                ...
                logger.debug(f"Coordination round {round_num + 1} completed in {round_duration:.1f}ms")
        ...
        coordination_mps = metrics.messages_sent / metrics.test_duration_seconds if metrics.test_duration_seconds > 0 else 0
    ...
    async def test_connection_stability_under_load(
        ...
        avg_mps = metrics.messages_sent / metrics.test_duration_seconds if metrics.test_duration_seconds > 0 else 0
    ...
    async def _maintain_stable_connection(
        ...
        while time.perf_counter() < end_time:
            try:
                ...
                logger.warning(f"Stable connection {connection_id} dropped, attempting reconnection")
...
async def test_websocket_connection_establishment_performance():
    ...
    metrics = await tester.test_connection_establishment_throughput(
        ...
        establishment_rate_per_second=5.0
    )
    
    # Validate performance requirements
    assert metrics.connections_established >= 18, f"Expected at least 18 connections, got {metrics.connections_established}"
...
async def test_websocket_message_throughput_realistic():
    ...
    assert metrics.messages_sent >= 200, f"Expected at least 200 messages sent, got {metrics.messages_sent}"
    
    # Validate latency requirements (P95 < 200ms from CLAUDE.md)
    assert metrics.p95_latency_ms < 200, f"P95 latency {metrics.p95_latency_ms:.1f}ms exceeds 200ms threshold"
...
async def test_websocket_multi_agent_coordination_throughput():
    ...
    assert metrics.connections_established >= 2, f"Expected at least 2 agent connections, got {metrics.connections_established}"
    assert metrics.messages_sent >= 20, f"Expected at least 20 coordination messages, got {metrics.messages_sent}"
    
    # Validate coordination latency (should be responsive for real-time coordination)
    assert metrics.average_latency_ms < 100, f"Average coordination latency {metrics.average_latency_ms:.1f}ms too high"
...
async def test_websocket_connection_stability_under_load():
    ...
    assert metrics.connections_established >= 4, f"Expected at least 4 stable connections, got {metrics.connections_established}"
    ...
    if metrics.connection_dropouts > 0:
        ...
        assert reconnection_rate >= 0.8, f"Reconnection rate {reconnection_rate:.1f} below 80% threshold"
