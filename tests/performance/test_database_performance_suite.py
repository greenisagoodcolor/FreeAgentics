...
class ConnectionPoolBenchmark:
    ...
    async def test_pool_size_formula(
        self, num_cores: int = 4, io_wait: float = 0.1
    ) -> Dict[str, Any]:
        """Test CLAUDE.md connection pool formula: (num_cores * 2) + effective IO wait."""
        recommended_size = int(
            (num_cores * 2) + (io_wait * 100)
        )  # Convert IO wait to reasonable number
        test_sizes = [
...
class MultiAgentTransactionBenchmark:
    ...
    async def test_belief_state_updates(
        self, num_agents: int = 100, concurrent_updates: int = 20
    ) -> Dict[str, Any]:
            ...
            try:
                ...
                for agent_id in batch_agent_ids:
                    ...
                    try:
                        ...
                        session.rollback()
                        if (
                            "serialization failure" in str(e).lower()
        ...
        stats.update(
            {
                ...
                "success_rate": success_count / (success_count + conflict_count)
                if (success_count + conflict_count) > 0
                else 0,
            }
        )
    ...
    async def test_coalition_formation(
        self, num_agents: int = 50, num_coalitions: int = 10
    ) -> Dict[str, Any]:
        ...
        def form_coalition_batch(coalition_batch):
            ...
            try:
                for coalition_id in coalition_batch:
                    ...
                    coalition = (
                        session.query(Coalition).filter(Coalition.id == coalition_id).first()
                    )
                    agents_to_add = (
                        session.query(Agent).filter(Agent.id.in_(selected_agent_ids)).all()
                    )
        ...
        batches = [
            coalition_ids[i : i + batch_size] for i in range(0, len(coalition_ids), batch_size)
        ]
    ...
    async def run_comprehensive_benchmark(self) -> Dict[str, Any]:
        ...
        try:
            ...
            pool_benchmark = ConnectionPoolBenchmark(
                self.engine.url.render_as_string(hide_password=False), use_sqlite=self.use_sqlite
            )
            ...
            self.test_results[
                "belief_updates"
            ] = await transaction_benchmark.test_belief_state_updates(
                num_agents=200, concurrent_updates=10
            )

            self.test_results[
                "coalition_formation"
            ] = await transaction_benchmark.test_coalition_formation(
            ...
            self.test_results[
                "query_performance"
            ] = await query_benchmark.test_complex_queries_with_analysis()
