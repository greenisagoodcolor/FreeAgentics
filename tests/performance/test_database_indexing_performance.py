...
class IndexPerformanceAnalyzer:
            ...
            for query_test in query_tests:
                ...
                if valid_times:
                    analysis_results[query_name] = {
                        ...
                        "is_slow": (sum(valid_times) / len(valid_times))
                        > 0.030,  # >30ms per CLAUDE.md
            ...
            if not self.use_sqlite:
                ...
                for stat in index_stats:
                    index_usage_stats[stat.indexname] = {
                        ...
                        "scans": stat.idx_scan,
                        "usage_efficiency": (
                            stat.idx_tup_fetch / stat.idx_tup_read if stat.idx_tup_read > 0 else 0
                        ),
                    }
    ...
    def generate_optimization_recommendations(
        self, query_analysis: Dict[str, Any], index_usage: Dict[str, Any]
    ) -> List[str]:
        ...
        slow_queries = [
            name
            for name, data in query_analysis.items()
        ...
        if missing_indexes:
            recommendations.append(f"Missing recommended indexes: {', '.join(missing_indexes)}")
...
class TestDatabaseIndexingPerformance:
    ...
    async def test_create_performance_indexes(self, index_analyzer):
        ...
        for required_index in required_indexes:
            ...
            assert results[required_index].get(
                "success", False
            ), f"Required index {required_index} failed to create"
    ...
    async def test_query_performance_analysis(self, index_analyzer):
        ...
        for query_name, query_data in analysis.items():
            if isinstance(query_data, dict) and "avg_time_ms" in query_data:
                ...
                assert (
                    query_data["avg_time_ms"] < 5000
                ), f"Query {query_name} too slow: {query_data['avg_time_ms']}ms"
    ...
    async def run_indexing_performance_demo():
        ...
        try:
            ...
            successful_count = sum(
                1 for data in index_results.values() if data.get("success", False)
            )
