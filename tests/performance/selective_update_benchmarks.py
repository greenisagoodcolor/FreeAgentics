...
class SparseObservationBenchmark(PyMDPBenchmark):
    ...
    def __init__(
        self, state_size: int = 30, sparsity_level: float = 0.1, selective_enabled: bool = True
    ):
    ...
    def run_iteration(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "sparsity_achieved": len(changed_modalities) / len(new_observations),
            "operations_skipped": len(new_observations) - len(changed_modalities),
        }
...
class PartialPolicyUpdateBenchmark(PyMDPBenchmark):
    ...
    def __init__(self, state_size: int = 20, num_policies: int = 50, update_fraction: float = 0.3):
    ...
    def setup(self):
        ...
        self.agent = PyMDPAgent(
            A_matrices, B_matrices, C=C_matrices, planning_horizon=3, inference_horizon=1
        )
    ...
    def run_iteration(self) -> Dict[str, Any]:
        ...
        observations = [np.random.randint(0, self.state_size) for _ in range(2)]
        self.agent.infer_states(observations)
    ...
    def _determine_policies_to_update(self, changed_factors: Set[int]) -> List[int]:
        """Determine which policies need updating based on changed state factors."""
        # Simulate policy dependency on state factors
        policies_to_update = []

        num_policies_affected = max(1, int(self.num_policies * self.update_fraction))
    ...
    def _compute_policy_value(self, policy_idx: int):
        ...
        for _ in range(10):  # Simulated computation steps
            ...
            dummy_computation = np.random.rand(self.state_size, self.state_size)
...
class IncrementalFreeEnergyBenchmark(PyMDPBenchmark):
    ...
    def _calculate_fe_component(
        self, factor_idx: int, beliefs: List[np.ndarray], observations: List[int]
    ) -> float:
    ...
    def _detect_belief_changes(
        ...
        for i, (curr, prev) in enumerate(zip(current_beliefs, previous_beliefs)):
...
class HierarchicalUpdateBenchmark(PyMDPBenchmark):
    ...
    def run_iteration(self) -> Dict[str, Any]:
        ...
        if self.update_propagation == "selective":
            ...
            computation_savings = (1 - levels_updated / self.hierarchy_levels) * 100
            speedup = full_time / selective_time if selective_time > 0 else 1.0

        else:
            # Full propagation only
            start_time = time.perf_counter()
            levels_updated = self.hierarchy.full_update(change_level, change_magnitude)
            full_time = (time.perf_counter() - start_time) * 1000
...
class ChangeTracker:
    ...
    def track_change(self, key: str, value: Any) -> bool:
        ...
        changed = key not in self.previous_values or self.previous_values[key] != value
...
class HierarchicalModel:
    ...
    def __init__(self, levels: int):
        self.levels = levels
        self.level_states = {}
        self.update_thresholds = [0.1, 0.2, 0.3]  # Thresholds for propagating updates

    def initialize(self):
        """Initialize hierarchical structure."""
        for level in range(self.levels):
            self.level_states[level] = np.random.rand(10)  # Random initial state

    def selective_update(self, change_level: int, change_magnitude: float) -> int:
...
def run_selective_update_benchmarks():
    ...
    print(f"{'='*70}")
    print("Testing optimization benefits of selective updates in PyMDP operations")
    ...
    hierarchical_results = [r for r in results if "hierarchical_update" in r.name]
    ...
    if fe_results:
        ...
        if incremental_results and full_fe_results:
            ...
            print(f"  Average full calculation time: {avg_full_fe_time:.2f} ms")
    ...
    if hierarchical_results:
        ...
        selective_hier = [r for r in hierarchical_results if "selective" in r.name]
        ...
        if selective_hier and full_hier:
            ...
            print(f"  Average full propagation time: {avg_full_hier_time:.2f} ms")
            ...
            if propagation_efficiencies:
                ...
                print(f"  Average propagation efficiency: {avg_prop_efficiency:.2f}")

                if avg_prop_efficiency <= 0.4:
                    print("  ✅ Excellent selective propagation (≤40% levels updated)")
                elif avg_prop_efficiency <= 0.6:
                    print("  ✅ Good selective propagation (≤60% levels updated)")
                elif avg_prop_efficiency <= 0.8:
                    print("  ⚠️  Moderate selective propagation (≤80% levels updated)")
                else:
                    print("  ❌ Poor selective propagation (>80% levels updated)")
    ...
    for result in results:
        if result.additional_metrics:
            ...
            savings = result.additional_metrics.get("computation_savings_percent")
    ...
    if all_savings:
        ...
        print(f"  Overall average computation savings: {overall_avg_savings:.1f}%")
