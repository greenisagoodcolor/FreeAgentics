...
class MemoryTracker:
    ...
    def _take_snapshot(self) -> MemorySnapshot:
        ...
        # Get GC stats
        gc_stats = {f"gen{i}": gc.get_count()[i] for i in range(gc.get_count().__len__())}
    ...
    def check_memory_leak(
        self, start: MemorySnapshot, end: MemorySnapshot, threshold_mb: float = 1.0
    ) -> bool:
        ...
        gc_ineffective = (
            all(end.gc_stats.get(f"gen{i}", 0) > start.gc_stats.get(f"gen{i}", 0) for i in range(3))
...
class MemoryDecorator:
    ...
    def _generate_report(
        ...
        gc_collections = {
            gen: end.gc_stats.get(gen, 0) - start.gc_stats.get(gen, 0) for gen in start.gc_stats
        }

        # Generate optimization suggestions
        suggestions = self._generate_suggestions(
            memory_delta, end.peak_mb, leak_detected, gc_collections
        )
    ...
    def _generate_suggestions(
        self, delta_mb: float, peak_mb: float, leak: bool, gc_stats: Dict[str, int]
    ) -> List[str]:
...
class AgentMemoryValidator:
    ...
    def validate_agent(self, agent_id: str) -> Tuple[bool, str]:
        ...
        if memory_mb > self.budget_mb:
            ...
            return True, (
                f"OK: Agent {agent_id} uses {memory_mb:.1f}MB " f"({usage_percent:.0f}% of budget)"
            )
