...
class TestGMNGenerationPipeline:
    ...
    async def test_complete_generation_pipeline(self, gmn_generator, gmn_repository, sample_prompt):
        """Test the complete pipeline from prompt to storage."""
        # Step 1: Generate GMN from natural language
        agent_id = uuid.uuid4()

        gmn_spec = await gmn_generator.prompt_to_gmn(prompt=sample_prompt, agent_type="explorer")
    ...
    async def test_versioning_pipeline(self, gmn_generator, gmn_repository, sample_prompt):
        """Test GMN versioning through refinement."""
        agent_id = uuid.uuid4()

        # Generate initial GMN
        initial_gmn = await gmn_generator.prompt_to_gmn(prompt=sample_prompt, agent_type="explorer")
        ...
        refined_gmn = await gmn_generator.refine_gmn(
            initial_gmn, "Add preference nodes to define exploration goals"
        )
    ...
    async def test_validation_and_error_handling(self, gmn_generator, gmn_repository):
        ...
        try:
            ...
            gmn_spec = await gmn_generator.prompt_to_gmn(
                prompt=minimal_prompt, agent_type="general"
            )
    ...
    async def test_concurrent_generation_pipeline(self, gmn_generator, gmn_repository):
        ...
        async def generate_and_store(agent_id, prompt, agent_type):
            ...
            # Generate GMN
            gmn_spec = await gmn_generator.prompt_to_gmn(prompt=prompt, agent_type=agent_type)
            ...
            stored_spec = gmn_repository.create_gmn_specification_versioned(
                ...
                parsed_data=self._parse_gmn_for_storage(gmn_spec),
                version_metadata={"agent_type": agent_type, "original_prompt": prompt},
            )
    ...
    async def test_rollback_pipeline(self, gmn_generator, gmn_repository):
        """Test the rollback functionality in the pipeline."""
        agent_id = uuid.uuid4()

        # Create initial version
        initial_gmn = await gmn_generator.prompt_to_gmn(
            prompt="Create a simple navigation agent", agent_type="explorer"
        )
        ...
        problematic_gmn = await gmn_generator.refine_gmn(
            initial_gmn, "Make this agent overly complex and hard to understand"
        )
    ...
    def _parse_gmn_for_storage(self, gmn_spec: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            if line.startswith("node "):
                parts = line.split()
                if len(parts) >= 3:
                    node_info = {"name": parts[2].rstrip("{"), "type": parts[1]}
