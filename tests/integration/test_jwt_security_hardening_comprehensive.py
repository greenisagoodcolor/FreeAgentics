...
class TestJWTSecurityIntegration:
    ...
    def test_secure_cookie_settings(self, client, test_user):
        ...
        response = client.post(
            "/api/v1/auth/login", json={"username": "testuser", "password": "SecurePassword123!"}
        )
    ...
    def test_logout_revokes_all_tokens(self, client, test_user):
        ...
        csrf_response = client.get(
            "/api/v1/auth/csrf-token", headers={"Authorization": f"Bearer {access_token}"}
        )
    ...
    def test_rate_limiting_on_auth_endpoints(self, client):
        ...
        for i in range(11):
            response = client.post(
                "/api/v1/auth/login", json={"username": f"user{i}", "password": "wrong"}
            )
    ...
    def test_comprehensive_security_flow(self, client):
        ...
        login_response = client.post(
            "/api/v1/auth/login",
            json={"username": "secureuser", "password": "VerySecurePassword123!"},
        )
        assert login_response.status_code == 200
