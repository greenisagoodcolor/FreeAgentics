...
from auth.security_logging import SecurityEventType, SecurityEventSeverity
...
class TestSecurityMonitoringSystem:
    ...
    async def test_ddos_detection(self):
        ...
        ddos_alerts = [
            alert for alert in self.security_monitor.active_alerts.values()
    ...
    async def test_alert_management(self):
        ...
        # Get alert ID
        alert_id = list(self.security_monitor.active_alerts.keys())[0] if self.security_monitor.active_alerts else None
    ...
    async def test_incident_lifecycle(self):
        ...
        # Resolve incident
        success = self.incident_response.resolve_incident(incident.id, "Attack blocked, vulnerability patched")
        assert success
    ...
    async def test_threat_indicator_matching(self):
        ...
        assert AttackType.SQL_INJECTION in alert_types or AttackType.SUSPICIOUS_ACTIVITY in alert_types
