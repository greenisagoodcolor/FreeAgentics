...
from inference.engine.diagnostics import DiagnosticConfig, DiagnosticSuite
...
from agents.base.data_model import AgentGoal, Position
...
# GNN integration requires torch_geometric
try:
    from inference.engine.gnn_integration import GNNActiveInferenceIntegration, GNNIntegrationConfig

    GNN_AVAILABLE = True
except ImportError:
    GNNActiveInferenceIntegration = None
    GNNIntegrationConfig = None
    GNN_AVAILABLE = False
...
class TestBasicIntegration:
    ...
    def test_discrete_inference_cycle(self) -> None:
        ...
        for t in range(10):
            ...
            belief = inference.infer_states(
                observation_tensor, gen_model, prior=belief)
            # Create a simple policy for testing
            test_policy = Policy([0])  # Single action policy
            policy_selector.compute_expected_free_energy(
                test_policy, belief, gen_model)
            selected_policy, action_probs = policy_selector.select_policy(
                belief, gen_model)
    ...
    def test_continuous_inference_cycle(self) -> None:
        ...
        for t in range(5):
            ...
            mean, particles, weights = pf.infer_states(
                obs, gen_model, prior=state)
            ...
            state = next_mean + \
                torch.randn_like(next_mean) * torch.sqrt(next_var)
...
class TestLearningIntegration:
    ...
    def test_online_learning_cycle(self) -> None:
        ...
        learner = create_parameter_learner(
            "online", learning_config, generative_model=gen_model)
        inference = VariationalInference(InferenceConfig(use_gpu=False))
        belief = torch.ones(4) / 4
        for t in range(50):
    ...
    def test_active_learning_integration(self) -> None:
        ...
        for t in range(30):
            ...
            active_learner.update_novelty_memory(
                belief.unsqueeze(0), observation_tensor)
...
class TestFullSystemIntegration:
    ...
    def test_complete_agent_simulation(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            print(
                f"DEBUG: Initial B[:, :, 0] shape: {gen_model.B[:, :, 0].shape}")
            inference = VariationalInference(InferenceConfig(use_gpu=False))
            policy_selector = DiscreteExpectedFreeEnergy(
                PolicyConfig(precision=1.0), inference)
            # DEBUG: Check B matrix after policy selector creation
            print(f"DEBUG: After policy selector B shape: {gen_model.B.shape}")
            print(
                f"DEBUG: After policy selector B[:, :, 0] shape: {gen_model.B[:, :, 0].shape}")
            precision_controller = PrecisionController(PrecisionConfig(
                init_precision=1.0), num_modalities=dims.num_observations)
            # DEBUG: Check B matrix after precision controller creation
            print(
                f"DEBUG: After precision controller B shape: {
                    gen_model.B.shape}")
            print(
                f"DEBUG: After precision controller B[:, :, 0] shape: {gen_model.B[:, :, 0].shape}"
            )
            planning_config = PlanningConfig(
                max_depth=5, branching_factor=3, search_type="mcts")
            planner = MonteCarloTreeSearch(
                planning_config, policy_selector, inference)
            # DEBUG: Check B matrix after planner creation
            print(f"DEBUG: After planner B shape: {gen_model.B.shape}")
            print(
                f"DEBUG: After planner B[:, :, 0] shape: {gen_model.B[:, :, 0].shape}")
            param_config = ParamLearningConfig(
                learning_rate_A=0.01, use_experience_replay=True)
            ...
            print(
                f"DEBUG: After param learner B[:, :, 0] shape: {gen_model.B[:, :, 0].shape}")
            active_config = ActiveLearningConfig(exploration_weight=0.3)
            _ = ActiveLearningAgent(
            ...
            for t in range(100):
                ...
                observation_tensor = torch.tensor(obs, dtype=torch.long)
                belief = optimizer.optimized_belief_update(
                    belief, observation_tensor, gen_model.A)
                pred_error = 1.0 - belief[true_state]
                precision = precision_controller.optimize(
                    torch.tensor([pred_error]))
                if t % 10 == 0:
                ...
                if action_sequence is not None:
                    ...
                    # Simple free energy calculation: G = action_idx + random
                    # noise
                    ...
                    G_adjusted = G_values
                    # Use only the first precision value to match G_values
                    # dimensions
                    precision_scalar = precision[0] if precision.numel(
                    ) > 1 else precision
                    action_probs = F.softmax(-G_adjusted *
                                             precision_scalar, dim=0)
                    action_idx = int(torch.multinomial(action_probs, 1).item())
                action_history.append(action_idx)
                next_belief = torch.matmul(
                    gen_model.B[:, :, action_idx], belief)
                # Check if param_learner has observe method
    ...
    def test_multi_agent_scenario(self) -> None:
        ...
        dims1 = ModelDimensions(
            num_states=4,
            num_observations=3,
            num_actions=2)
        dims2 = ModelDimensions(
            num_states=3,
            num_observations=4,
            num_actions=2)
        params = ModelParameters(use_gpu=False)
        ...
        agent1_policy = DiscreteExpectedFreeEnergy(
            PolicyConfig(precision=1.0), agent1_inference)
        agent2_model = DiscreteGenerativeModel(dims2, params)
        agent2_inference = VariationalInference(InferenceConfig(use_gpu=False))
        agent2_policy = DiscreteExpectedFreeEnergy(
            PolicyConfig(precision=3.0), agent2_inference)
        ...
        for t in range(20):
            ...
            belief1 = agent1_inference.infer_states(
                observation_tensor, agent1_model, prior=belief1)
            # Create policies for each possible action
            policies = [
                Policy(
                    [a]) for a in range(
                    agent1_model.dims.num_actions)]
            G_values = []
            ...
            belief2 = agent2_inference.infer_states(
                observation_tensor, agent2_model, prior=belief2)
            # Create policies for each possible action
            policies2 = [
                Policy(
                    [a]) for a in range(
                    agent2_model.dims.num_actions)]
            G_values2 = []
...
class TestErrorHandling:
    ...
    def test_recovery_from_errors(self) -> None:
        ...
        try:
            ...
            updated = belief_updater.update_beliefs(
                safe_belief, torch.tensor(obs), gen_model)
...
class TestActiveInferenceIntegration:
    ...
    def components(self, agent):
        ...
        pathfinding_grid = PathfindingGrid(
            width=100, height=100, cell_size=1.0)
    ...
    def test_integration_creation(self, agent, components, ai_config) -> None:
        ...
        integration = create_active_inference_agent(
            agent, **components, config=ai_config)
    ...
    def test_state_to_belief_mapping(
            self, agent, components, ai_config) -> None:
        ...
        belief = integration.state_mapper.map_to_belief(
            agent, components["state_manager"])
    ...
    def test_perception_to_observation_mapping(
            self, agent, components, ai_config) -> None:
        ...
        observation = integration.perception_mapper.map_to_observation(
            percepts)
    ...
    def test_full_integration_update(
            self, agent, components, ai_config) -> None:
    ...
    def test_hybrid_mode_integration(
        ...
        for _ in range(5):
            integration.update(dt=0.1)
        memories = components["memory_system"].retrieve_memories(
            {"memory_type": MemoryType.EPISODIC}, 5
        )
        assert len(memories) > 0

    def test_advisory_mode_integration(
            self, agent, components, ai_config) -> None:
    ...
    def test_learning_mode_integration(
            self, agent, components, ai_config) -> None:
    ...
    @pytest.mark.xfail(reason="Belief evolution not properly implemented yet")
        ...
        for i in range(10):
            ...
            if integration.current_belief is not None:
                ...
                if initial_belief is None:
                    ...
                    print(f"Initial belief shape: {initial_belief.shape}")
                else:
                    change = np.linalg.norm(current_belief_np - initial_belief)
                    belief_changes.append(change)
                    # Debug output
                    if i < 3:  # Print first few changes
                        print(f"Step {i}: belief change = {change:.6f}")
            else:
                print(f"Step {i}: belief is None")
        # Verify belief evolution
        assert len(belief_changes) > 0
        # Check for any belief change (even small ones)
        assert any(
            change > 1e-6 for change in belief_changes), "Beliefs should evolve over time"
        # Also check that beliefs don't always stay exactly the same
        assert not all(
            change == 0.0 for change in belief_changes
        ), "Beliefs should not remain static"
    ...
    @pytest.mark.xfail(reason="Memory integration not properly implemented yet")
    def test_memory_integration(self, agent, components, ai_config) -> None:
        ...
        for _ in range(5):
            integration.update(dt=0.1)
        memories = components["memory_system"].retrieve_memories(
            {"memory_type": "episodic"}, 10)
        experience_memories = [
            m for m in memories if m.memory_type == "episodic"]
    ...
    def test_multi_agent_integration(self, components) -> None:
        ...
        for i in range(3):
            ...
            move_ctrl = MovementController(
                state_mgr, collision_sys, pathfinding)
