...
class GNNToLLMTransformer:
    ...
    def embeddings_to_text(
        self, embeddings: np.ndarray, node_ids: List[str], graph_metadata: Dict[str, Any]
    ) -> str:
        """
        ...
        description_parts.append(f"- Medium activity nodes: {medium_activity_count}")
        ...
        if len(node_ids) == len(embeddings):
            ...
            top_indices = np.argsort(importance_scores)[-min(3, len(node_ids)) :]
    ...
    def validate_transformation_quality(
        ...
        validation_results = {
            ...
            "has_numerical_data": any(char.isdigit() for char in text_description),
            "reasonable_length": 100 < len(text_description) < 2000,
            "structured_format": "\n" in text_description and "-" in text_description,
        }

        validation_results["overall_quality"] = sum(validation_results.values()) / len(
            validation_results
        )
...
class LLMGraphReasoningValidator:
    ...
    async def validate_llm_reasoning(
        ...
        """
        if not self.llm_manager or reasoning_type not in self.reasoning_prompts:
            return {
                "success": False,
        ...
        try:
            ...
            return {"success": False, "error": str(e), "reasoning_quality": 0.0}
...
class TestGNNLLMInterfaceIntegration:
    ...
    async def test_graph_data(self):
        ...
        nodes = [
            ("agent_1", [0, 0, 1, 50, 0.8, 0.9]),  # type=1 (agent), high connectivity/importance
            ("agent_2", [10, 5, 1, 40, 0.6, 0.7]),  # type=1 (agent), medium connectivity
            ("agent_3", [15, 15, 1, 30, 0.4, 0.5]),  # type=1 (agent), lower connectivity
            ("resource_1", [5, 3, 2, 100, 0.9, 0.8]),  # type=2 (resource), high value
            ("resource_2", [12, 8, 2, 80, 0.7, 0.6]),  # type=2 (resource), medium value
            ("obstacle_1", [7, 7, 3, 0, 0.1, 0.2]),  # type=3 (obstacle), low connectivity
            ("base_1", [2, 2, 4, 200, 0.95, 0.95]),  # type=4 (base), very high importance
        ]

        edges = [
            (0, 1, [7.07, 0.8, 1]),  # agent_1 → agent_2: distance, weight, type
            (0, 3, [5.83, 0.9, 2]),  # agent_1 → resource_1: high weight connection
            (1, 4, [4.47, 0.7, 2]),  # agent_2 → resource_2: medium weight connection
            (2, 4, [7.07, 0.5, 2]),  # agent_3 → resource_2: lower weight
            (0, 6, [2.83, 0.95, 3]),  # agent_1 → base_1: very high weight
            (5, 3, [4.47, 0.1, 4]),  # obstacle_1 blocks resource_1: low weight (blockage)
            (5, 4, [6.40, 0.1, 4]),  # obstacle_1 affects resource_2: low weight
        ]
    ...
    async def test_gnn_embedding_generation(self, gnn_model, test_graph_data):
        ...
        try:
            ...
            gnn_output = gnn_model.forward(node_features, edge_indices, edge_features)
            ...
            embedding_norms = np.linalg.norm(embeddings, axis=1)
            assert np.all(embedding_norms > 0), "Some embeddings have zero norm"
            assert np.std(embedding_norms) > 0, "All embeddings have identical norms"

            logger.info(f"✓ GNN generated valid embeddings: shape {embeddings.shape}")
            logger.info(
    ...
    async def test_embedding_to_text_transformation(self, test_graph_data):
        ...
        logger.info(f"  Quality score: {validation_results['overall_quality']:.3f}")
    ...
    async def test_llm_graph_reasoning(self, llm_manager, test_graph_data):
        ...
        reasoning_types = ["strategy_analysis", "risk_assessment", "optimization_opportunities"]
        reasoning_results = {}
        ...
        assert len(successful_reasoning) > 0, "No LLM reasoning attempts succeeded"
            ...
            if reasoning_result["success"]:
                ...
                semantic_checks = {
                    ...
                    "preserves_spatial_concept": any(
                        ...
                        for term in ["distance", "location", "position", "spatial"]
                    ),
                ...
                logger.warning("LLM reasoning failed, testing text transformation only")
    ...
    async def test_integration_performance_characteristics(
        ...
        performance_results["gnn_processing"] = {
            ...
            "throughput_nodes_per_sec": (
                len(test_graph_data["node_features"]) / gnn_time if gnn_time > 0 else 0
            ),
        }
        ...
        text_description = transformer.embeddings_to_text(
            embeddings, test_graph_data["node_ids"], test_graph_data["metadata"]
        )
        ...
        performance_results["text_transformation"] = {
            ...
            "throughput_chars_per_sec": (
                len(text_description) / transform_time if transform_time > 0 else 0
            ),
        }
        ...
        if llm_manager:
            ...
            performance_results["llm_reasoning"] = {
                ...
                "reasoning_quality": reasoning_result.get("reasoning_quality", 0),
            }
        ...
        performance_results["overall_pipeline"] = {
            ...
            "end_to_end_success": all(
                result.get("success", True) for result in performance_results.values()
            ),
        }
    ...
    async def test_integration_edge_cases(self, gnn_model, test_graph_data):
        ...
        extreme_embeddings = np.array(
            [
                ...
                [np.inf, 0, 0, 0, 0, 0],  # Infinite values (if not caught by GNN)
            ],
            dtype=np.float32,
        )
        ...
        try:
            extreme_text = transformer.embeddings_to_text(
                extreme_embeddings, ["extreme_1", "extreme_2", "extreme_3"], {"type": "stress_test"}
            )
        ...
        large_embeddings = np.random.randn(large_graph_size, 6).astype(np.float32)
    ...
    async def run_interface_tests():
        ...
        test_graph_data = {
            ...
            "edge_features": np.array(
                [[7.07, 0.8, 1], [5.83, 0.9, 2], [2.83, 0.95, 3]], dtype=np.float32
            ),
            "node_ids": ["agent_1", "agent_2", "resource_1", "base_1"],
            "metadata": {"scenario": "test", "agent_count": 2, "resource_count": 1},
        }
