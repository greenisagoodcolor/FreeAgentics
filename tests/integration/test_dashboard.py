...
TEST_CATEGORIES = {
    ...
    "performance": {
        ...
        "tests": ["test_auth_load.py", "test_rbac_scale.py", "test_monitoring_load.py"],
    },
    "security": {
...
class IntegrationTestDashboard:
    ...
    def run_test(self, test_file: str, verbose: bool = False) -> Tuple[bool, str]:
        ...
        # Run test
        cmd = [self.venv_python, "-m", "pytest", test_path, "-v" if verbose else "-q", "--tb=short"]
...
def main():
    ...
    parser.add_argument(
        "command", choices=["check", "list", "run", "report"], help="Command to execute"
    )
