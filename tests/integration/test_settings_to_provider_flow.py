...
from auth.security_implementation import TokenData, Permission, UserRole
...
class TestSettingsToProviderFlow:
    ...
    def test_update_settings_creates_user_record(self, mock_auth, client, test_db, mock_user):
        ...
        settings = test_db.query(UserSettings).filter(
            UserSettings.user_id == mock_user.user_id
        ).first()
    ...
    def test_prompt_uses_user_settings(
        self, mock_openai_class, mock_auth, 
    ...
    def test_no_api_key_falls_back_to_mock(
        self, mock_auth, client, test_db, mock_user
    ):
