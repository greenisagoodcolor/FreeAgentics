...
from agents.base.movement import CollisionSystem
...
class TestAgentIntegration:
    ...
    def test_full_agent_lifecycle(self) -> None:
        ...
        perception_system.perception_capabilities[agent.agent_id].visual_range = 100.0
        perception_system.perception_capabilities[agent.agent_id].field_of_view = (
            math.pi * 2
        )  # 360 degrees
    ...
    def test_multi_agent_interaction(self) -> None:
        ...
        # Test persistence
        persistence = AgentPersistence()
        agent_data = persistence._serialize_agent(agent1)
        assert len(memory_system.retrieve_memories({"memory_type": MemoryType.EPISODIC}, 10)) > 0
