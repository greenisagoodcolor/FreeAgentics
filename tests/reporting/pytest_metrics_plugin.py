...
class MetricsPlugin:
    ...
    def pytest_sessionfinish(self, session: Session, exitstatus: int):
        ...
        if self.config.getoption("--generate-reports", False):
            ...
            print("Reports generated in tests/reporting/")
...
def pytest_configure(config: Config):
    """Configure the plugin, markers, and integrations."""
...
# Add additional option for disabling metrics - MERGED INTO MAIN pytest_addoption ABOVE
    ...
    if hasattr(config, "pluginmanager"):
        ...
        if plugin and hasattr(plugin, "collector"):
            ...
            if flaky_tests or slow_tests:
                ...
                terminalreporter.write_line(
                    "📊 Full metrics report: tests/reporting/metrics_report.html"
                )


# Hook for custom markers - MERGED INTO MAIN pytest_configure ABOVE
...
# Integration with external reporting tools - MERGED INTO MAIN pytest_configure ABOVE
