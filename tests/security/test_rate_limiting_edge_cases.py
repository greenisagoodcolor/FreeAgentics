...
class TestEdgeCaseHandling:
    ...
    async def test_malformed_requests(self, rate_limiter):
        ...
        malformed_requests = [
            ...
            # Missing host
            {"client": MagicMock(host=None), "headers": {}, "path": "/api/v1/test"},
            # Invalid path
            {"client": MagicMock(host="192.168.1.1"), "headers": {}, "path": None},
            # Circular reference in headers
            {"client": MagicMock(host="192.168.1.1"), "headers": {}, "path": "/test"},
        ]
