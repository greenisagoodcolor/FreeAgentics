...
class PerformanceUnderAttackTester:
    ...
    async def run_all_attack_scenarios(self) -> Dict:
        ...
        scenarios = [
            ...
            ("Authentication Bypass Attempts", self._simulate_auth_bypass_flood),
    ...
    async def _simulate_rate_limit_stress(self) -> AttackMetrics:
        ...
        for _ in range(1000):
            try:
                ...
                if response.status_code in [200, 401, 429]:  # Including rate limit response
                    successful_requests += 1
    ...
    def _calculate_performance_impact(self, metrics: AttackMetrics) -> Dict:
        ...
        attack_avg_response = statistics.mean(metrics.response_times) if metrics.response_times else 0
        ...
        return {
            "response_time_increase": ((attack_avg_response - baseline_avg_response) / baseline_avg_response) * 100,
            "success_rate_decrease": ((self.baseline_metrics.success_rate - metrics.success_rate) / self.baseline_metrics.success_rate) * 100,
            "throughput_decrease": ((self.baseline_metrics.throughput - metrics.throughput) / self.baseline_metrics.throughput) * 100,
    ...
    def _generate_recommendations(self, scenario: str, metrics: AttackMetrics) -> List[str]:
        ...
        if metrics.response_times:
            ...
            if avg_response > self.thresholds["max_response_time"]:
                recommendations.append(f"Response time ({avg_response:.2f}s) exceeds threshold. Consider implementing caching or optimizing queries.")
        
        if metrics.success_rate < self.thresholds["min_success_rate"]:
            recommendations.append(f"Success rate ({metrics.success_rate:.2%}) below threshold. Implement better error handling and rate limiting.")
        ...
        if "SQL Injection" in scenario:
            recommendations.append("Ensure parameterized queries and input validation are properly implemented.")
    ...
    def _generate_attack_performance_report(self, results: Dict) -> Dict:
        ...
        report = {
            ...
            "baseline_metrics": {
                "avg_response_time": statistics.mean(self.baseline_metrics.response_times) if self.baseline_metrics else 0,
    ...
    def _generate_overall_recommendations(self, results: Dict) -> List[str]:
        ...
        failed_scenarios = [name for name, result in results.items() if not result.get("passed", False)]
        
        if len(failed_scenarios) > 3:
            recommendations.append("Multiple attack scenarios failed. Consider comprehensive security hardening.")
        
        if any("DDoS" in scenario for scenario in failed_scenarios):
            recommendations.append("Implement DDoS protection measures (CloudFlare, AWS Shield, etc.).")
        
        if any("Brute Force" in scenario for scenario in failed_scenarios):
            recommendations.append("Strengthen authentication mechanisms and implement account protection.")
...
if __name__ == "__main__":
    async def main():
        ...
        print(f"Passed Scenarios: {report['summary']['passed_scenarios']}/{report['summary']['total_scenarios']}")
