...
from tests.security.run_cryptography_assessment import CryptographyAssessmentRunner
...
class ComprehensiveCryptoSecuritySuite:
    ...
    def _run_dynamic_testing(self) -> Dict[str, Any]:
        ...
        try:
            runner = CryptographyAssessmentRunner(
                output_dir=str(self.output_dir / "dynamic"), output_format="json"
            )
            ...
            if "raw_findings" in dynamic_report:
                for finding in dynamic_report["raw_findings"]:
                    self.consolidated_findings.append(
                        {"source": "dynamic_testing", "category": "implementation", **finding}
                    )
    ...
    def _run_static_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "failed", "error": str(e), "vulnerabilities_found": 0}
    ...
    def _map_findings_to_standards(self) -> Dict[ComplianceStandard, List[Dict]]:
        """Map findings to compliance standards."""
    ...
    def _identify_top_risks(self) -> List[Dict[str, Any]]:
        ...
        top_risks.sort(key=lambda x: (x["severity"] == "critical", x["count"]), reverse=True)
        return top_risks[:10]
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Run quick assessment (skip some time-intensive tests)"
    )
