...
from api.middleware.rate_limiter import (
    RateLimiter, RateLimitConfig, EndpointConfig,
    RateLimitAlgorithm, RateLimitType, BlockReason,
    SuspiciousPatternDetector, RateLimitMiddleware,
    create_rate_limiter
)
...
class TestRateLimiterIntegration:
    ...
    async def test_ddos_detection(self, rate_limiter, redis_client):
        ...
        for i in range(6):
            ...
            if i < 5:
                ...
                assert block_reason == BlockReason.SUSPICIOUS_PATTERN, "Should detect suspicious pattern"
    ...
    async def test_concurrent_requests(self, rate_limiter, redis_client):
        ...
        assert allowed_count <= max_allowed, f"Should allow at most {max_allowed} requests, got {allowed_count}"
