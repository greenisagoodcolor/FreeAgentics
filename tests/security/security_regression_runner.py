...
class SecurityRegressionRunner:
    ...
    async def _run_comprehensive_security_suite(self):
        ...
        try:
            ...
            process = await asyncio.create_subprocess_exec(
                sys.executable, "tests/security/comprehensive_security_test_suite.py",
            ...
            if report_file.exists():
                ...
                self.test_results["comprehensive_security_suite"] = {
                    "status": "PASSED" if report_data.get("summary", {}).get("vulnerabilities_found", 0) == 0 else "FAILED",
                    "vulnerabilities_found": report_data.get("summary", {}).get("vulnerabilities_found", 0),
            ...
            print(f"  ✓ Comprehensive Security Suite: {self.test_results['comprehensive_security_suite']['status']}")
    ...
    async def _run_penetration_tests(self):
        ...
        try:
            process = await asyncio.create_subprocess_exec(
                sys.executable, "tests/security/run_comprehensive_penetration_tests.py",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
                cwd="."
            )
    ...
    async def _run_performance_under_attack_tests(self):
        ...
        try:
            process = await asyncio.create_subprocess_exec(
                sys.executable, "tests/security/performance_under_attack.py",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
                cwd="."
            )
            ...
            print(f"  ✓ Performance Under Attack: {self.test_results['performance_under_attack']['status']}")
    ...
    async def _run_owasp_zap_scan(self):
        ...
        try:
            ...
            # Run ZAP integration
            process = await asyncio.create_subprocess_exec(
                sys.executable, "tests/security/owasp_zap_integration.py",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
                cwd="."
            )
    ...
    async def _run_security_headers_validation(self):
        ...
        try:
            process = await asyncio.create_subprocess_exec(
                sys.executable, "-m", "pytest", "tests/unit/test_security_headers_validation.py", "-v",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
                cwd="."
            )
    ...
    async def _run_rate_limiting_verification(self):
        ...
        try:
            process = await asyncio.create_subprocess_exec(
                sys.executable, "-m", "pytest", "tests/unit/test_rate_limiting_verification.py", "-v",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
                cwd="."
            )
    ...
    async def _run_rbac_comprehensive_tests(self):
        ...
        try:
            process = await asyncio.create_subprocess_exec(
                sys.executable, "-m", "pytest", "tests/integration/test_rbac_comprehensive.py", "-v",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
                cwd="."
            )
    ...
    async def _run_security_monitoring_tests(self):
        ...
        try:
            process = await asyncio.create_subprocess_exec(
                sys.executable, "-m", "pytest", "tests/integration/test_security_monitoring_system.py", "-v",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
                cwd="."
            )
    ...
    def _calculate_security_metrics(self) -> Dict:
        ...
        if self.test_results.get("comprehensive_security_suite"):
            ...
            if result.get("details"):
                metrics["vulnerability_count"] = result["details"].get("summary", {}).get("vulnerabilities_found", 0)
                metrics["security_score"] = result["details"].get("summary", {}).get("security_score", 0)
    ...
    def _check_compliance_status(self) -> Dict:
        ...
        if self.test_results.get("comprehensive_security_suite"):
            ...
            if result.get("details") and result["details"].get("compliance"):
                ...
                compliance["OWASP_Top_10"]["details"] = f"Passed {passed_checks}/{total_checks} checks"
...
<body>
    ...
    def _print_summary(self, report: Dict):
        ...
        # Recommendations
        high_priority_recs = [r for r in report['recommendations'] if r['priority'] in ['CRITICAL', 'HIGH']]
