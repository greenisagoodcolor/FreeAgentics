...
class TestCompleteOnboarding:
    ...
    def test_clone_and_run_complete_flow(self):
        ...
        install_result = subprocess.run(
            ["make", "install"], cwd=project_root, capture_output=True, text=True, timeout=120
        )
                ...
                if not frontend_ready:
                    for port in [3000, 3001, 3002, 3003]:
                        try:
                            ...
                            if frontend_response.status_code in [
                                200,
                                404,
                                500,
        ...
        try:
            ...
            ), f"Agents endpoint failed: {agents_response.status_code}"

            # Test knowledge graph endpoint
            kg_response = requests.get("http://localhost:8000/api/knowledge-graph", headers=headers)
            assert (
                kg_response.status_code == 200
            ), f"Knowledge graph endpoint failed: {kg_response.status_code}"
            ...
            create_response = requests.post(
                "http://localhost:8000/api/v1/agents", json=agent_data, headers=headers
            )
            assert (
