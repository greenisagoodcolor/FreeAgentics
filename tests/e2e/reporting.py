...
class TestResultCollector:
    ...
    def start_suite(self, suite_name: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        try:
            if self.current_suite is not None:
                logger.warning(
                    f"Starting new suite '{suite_name}' while '{self.current_suite.suite_name}' is active"
                )
                self.finish_suite()

            self.current_suite = TestSuiteResult(suite_name=suite_name, metadata=metadata or {})
    ...
    def add_test_passed(
        self, test_name: str, duration: float = 0.0, metadata: Optional[Dict[str, Any]] = None
    ) -> None:
        """Add a passed test result."""
        result = TestResult(
            test_name=test_name, status="passed", duration=duration, metadata=metadata or {}
        )
    ...
    def add_test_skipped(
        ...
        result = TestResult(
            test_name=test_name, status="skipped", error_message=reason, metadata=metadata or {}
        )
        self.add_result(result)
