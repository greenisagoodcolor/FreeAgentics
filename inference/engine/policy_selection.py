    ...
    def _get_pymdp_matrices(
        ...
        if hasattr(generative_model, "B"):
            ...
            B = torch.eye(
                ...
                device=self.device,
                dtype=self.config.dtype
            ).unsqueeze(-1).repeat(1, 1, generative_model.dims.num_actions)
    ...
    def _calculate_epistemic_value(
        ...
        for obs_idx in range(A.shape[0]):
            ...
            if obs_prob > self.eps:
                ...
                kl_div = torch.sum(
                    posterior * torch.log(
                        (posterior + self.eps) / (predicted_states + self.eps)
                    )
                )
...
if __name__ == "__main__":
    ...
    model.A = torch.eye(3, 4)  # Identity observation model
    model.B = torch.eye(4).unsqueeze(-1).repeat(1, 1, 2)  # Identity transition
    model.C = torch.tensor([[-1.0], [0.0], [2.0]])  # Preferences
    model.D = torch.ones(4) / 4  # Uniform prior
    ...
    print(f"Policy follows PyMDP conventions: {validate_gnn_policy_compatibility(policy, {'action_space': {'size': 2}})}")