...
class Policy:
    ...
    def __init__(
        ...
        """Initialize policy with actions and horizon."""
...
class DiscreteExpectedFreeEnergy(PolicySelector):
    ...
    def compute_expected_free_energy(
        ...
        if hasattr(generative_model, "B") and hasattr(generative_model.B, "to"):
            ...
            B = (
                torch.eye(generative_model.dims.num_states, device=self.device)
                .unsqueeze(-1)
        ...
        for t in range(min(len(policy), self.config.planning_horizon)):
            ...
            if self.config.pragmatic_weight > 0:
                ...
                # Ensure tensors have compatible shapes for multiplication
...
def create_policy_selector(
    ...
    """Create policy selectors.

    Args:
        selector_type: Type of selector ('discrete', 'continuous',
            'hierarchical', 'sophisticated')
...
# Backward compatibility: Replace DiscreteExpectedFreeEnergy with pymdp
# implementation
...
try:
    ...
    logger = logging.getLogger(__name__)
    logger.warning(
        f"Could not load pymdp policy selector: {e}, " f"keeping original implementation"
    )
