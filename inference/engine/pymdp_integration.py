...
from .generative_model import GenerativeModel, DiscreteGenerativeModel, ModelDimensions, ModelParameters
from .active_inference import VariationalMessagePassing, InferenceConfig
...
class PyMDPActiveInference:
    ...
    def _validate_pymdp_compatibility(self) -> None:
        ...
        if hasattr(self.generative_model, 'dims'):
            ...
            if A.shape != expected_shape:
                logger.warning(f"A matrix shape {A.shape} doesn't match pymdp convention {expected_shape}")
        ...
        if hasattr(self.generative_model, 'B'):
            ...
            if B.shape != expected_B_shape:
                logger.warning(f"B matrix shape {B.shape} doesn't match pymdp convention {expected_B_shape}")
                
    def update_beliefs(
        self, 
        observations: Union[int, torch.Tensor], 
    ...
    def compute_expected_free_energy(
        ...
        for t in range(time_horizon):
            ...
            predicted_obs = A @ predicted_states if predicted_states.dim() == 1 else (A @ predicted_states.T).T
            ...
            if predicted_obs.dim() == 1:
                ...
                entropy_obs = -torch.sum(predicted_obs * torch.log(predicted_obs + 1e-16), dim=1).sum()
...
def create_pymdp_agent(agent_config: Dict[str, Any]) -> PyMDPActiveInference:
    ...
    logger.info(f"Created pymdp-compatible agent: {agent_config['agent_name']} ({agent_config['agent_class']})")
