...
class GenerativeModel(ABC):
    ...
    @abstractmethod
    def observation_model(self, states: torch.Tensor) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        """Compute p(o|s) following PyMDP A matrix conventions."""
        pass

    @abstractmethod
    def transition_model(self, states: torch.Tensor, actions: torch.Tensor) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
    ...
    def validate_pymdp_matrices(self) -> bool:
        ...
        if not hasattr(self, 'A') or not hasattr(self, 'B') or not hasattr(self, 'C') or not hasattr(self, 'D'):
            return False
...
class DiscreteGenerativeModel(GenerativeModel):
    ...
    def transition_model(self, states: torch.Tensor, actions: Union[torch.Tensor, int]) -> torch.Tensor:
...
class ContinuousGenerativeModel(GenerativeModel, nn.Module):
    ...
    def transition_model(
        ...
        actions_was_1d = (actions.dim() == 1 and 
                         (actions.shape[0] == 1 or 
                          actions.shape[0] == self.dims.num_actions)) or \
        ...
        if (actions.dim() == 1 and actions.shape[0] == self.dims.num_actions and 
            actions.dtype.is_floating_point and torch.allclose(actions.sum(), torch.tensor(1.0))):
            ...
            actions_onehot = F.one_hot(actions_flat.long(), num_classes=self.dims.num_actions).float()
        else:
        ...
        for level in range(self.num_levels):
            ...
            for upper_level in range(level):
                if (upper_level, level) in self.E_matrices:
                    ...
                    next_state = next_state / (next_state.sum(dim=-1, keepdim=True) + self.params.eps)
...
def create_generative_model(model_type: str, **kwargs: Any) -> GenerativeModel:
    ...
    if model_type == "discrete":
        ...
        if None in [factor_dims, num_obs, num_actions]:
            raise ValueError("FactorizedGenerativeModel requires 'factor_dimensions', 'num_observations', and 'num_actions' parameters")
...
def _apply_gnn_initialization(model: GenerativeModel, gnn_spec: Dict[str, Any]) -> None:
    ...
    if isinstance(model, DiscreteGenerativeModel):
        ...
        if init_spec.get("A_matrix") == "identity_with_noise":
            model.A.data = torch.eye(model.dims.num_observations, model.dims.num_states) + \
