...
class PyMDPExtractionContext:
    ...
    def to_extraction_context(self) -> ExtractionContext:
        ...
        content = (
            f"Agent {self.agent_id} performed inference with action {self.inference_result.action}"
        )
...
class PyMDPEntityExtractor(EntityExtractionStrategy):
    ...
    def __init__(self, confidence_threshold: float = 0.1):
        ...
        logger.info(
            f"PyMDPEntityExtractor initialized with confidence threshold {confidence_threshold}"
        )
            ...
            if isinstance(states, list):
                ...
                for i, factor_beliefs in enumerate(states):
                    entity = self._create_belief_state_entity(
                        factor_beliefs, context, factor_index=i
                    )
                    if entity:
    ...
    def _create_belief_state_entity(
        ...
        try:
            ...
            # Create label
            label = (
                f"belief_state_factor_{factor_index}"
    ...
    def _extract_inference_entity(
        self, context: PyMDPExtractionContext
    ) -> Optional[ConversationEntity]:
        """Extract inference step entity."""
        try:
...
class PyMDPRelationExtractor(RelationExtractionStrategy):
    ...
    def __init__(self, temporal_window_ms: int = 1000):
        ...
        logger.info(
            f"PyMDPRelationExtractor initialized with {temporal_window_ms}ms temporal window"
        )
    ...
    def _extract_temporal_relations(
        ...
        for i in range(len(temporal_entities) - 1):
            ...
            if current_entity.temporal_metadata and next_entity.temporal_metadata:
                time_diff = (
                    next_entity.temporal_metadata.created_at
                    - current_entity.temporal_metadata.created_at
                ).total_seconds() * 1000
    ...
    def _extract_confidence_relations(
        ...
        high_confidence_entities = [
            e for e in entities if e.provenance and e.provenance.confidence_score > 0.7
        ]
