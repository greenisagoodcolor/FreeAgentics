...
import { Send, X, Loader2, CornerDownRight, AlertTriangle, Upload } from "lucide-react"
...
export default function ChatWindow({ conversation, agents, onSendMessage, onEndConversation }: ChatWindowProps) {
  const [message, setMessage] = useState("")
  const [userAgentId, setUserAgentId] = useState<string | "user">("user")
  const messagesContainerRef = useRef<HTMLDivElement>(null)
  const [localError, setLocalError] = useState<string | null>(null)
  const [isSending, setIsSending] = useState(false)
  const [processingMessageMap, setProcessingMessageMap] = useState<Record<string, boolean>>({})
  ...
  }, [conversation?.messages, agents, isSending, isProcessing, queueAgentResponse])
  ...
  useEffect(() => {
    if (!conversation || !conversation.messages || conversation.messages.length === 0 || isProcessing || isSending)
      return
    ...
    <div className="flex flex-col h-full">
      ...
      >
        {conversation && conversation.messages ? (
          conversation.messages.length > 0 ? (
            <div className="space-y-4">
                ...
                return (
                  <div key={msg.id} className="flex flex-col">
                    <div className="flex items-center gap-2">
                      <div className="w-3 h-3 rounded-full" style={{ backgroundColor: senderColor }} />
                      <span className="font-semibold text-purple-100">{senderName}</span>
                      <span className="text-xs text-purple-300">{new Date(msg.timestamp).toLocaleTimeString()}</span>
                      {msg.metadata?.isGeneratedByLLM && (
                        <span className="text-xs bg-purple-800/50 px-1.5 py-0.5 rounded text-purple-200">AI</span>
                      )}
                      {msg.metadata?.respondingTo && (
                        <div className="flex items-center text-xs text-purple-300">
                          ...
                          {getMessageById(msg.metadata.respondingTo)
                            ? getMessageById(msg.metadata.respondingTo)?.content.substring(0, 20) + "..."
                            : "a previous message"}
                        </div>
                      )}
                    </div>
              ...
              {Object.entries(typingAgents).map(([agentId, { text, messageId }]) => {
                ...
                return (
                  <div key={`typing-${agentId}`} className="flex flex-col opacity-80">
                    <div className="flex items-center gap-2">
          ...
          <div className="text-center text-purple-300 py-8">No active conversation. Add agents to start one.</div>
        )}
        ...
        <div className="p-4 border-t border-purple-800 bg-black/20 flex-shrink-0">
          <div className="flex gap-2 mb-2 items-center">
            ...
            >
              ...
              {agents
                .filter((agent) => conversation.participants && conversation.participants.includes(agent.id))
                .map((agent) => (
