...
import { ZoomIn, ZoomOut, ArrowUp, ArrowDown, ArrowLeft, ArrowRight, RefreshCw } from "lucide-react"
import GlobalKnowledgeGraph from "./global-knowledge-graph"

interface GridWorldProps {
  agents: Agent[]
  onUpdatePosition: (agentId: string, position: Position) => void
  activeConversation: Conversation | null
  onSelectKnowledgeNode: (nodeType: "entry" | "tag", nodeId: string, nodeTitle: string) => void
  onShowAbout: () => void
}
...
export default function GridWorld({
  ...
  useEffect(() => {
    ...
    ctx.restore()
  }, [agents, gridSize, cellSize, selectedAgentId, activeConversation, zoomLevel, viewportOffset])
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 flex flex-col" ref={containerRef}>
            ...
            <div className="flex-1 relative">
              ...
              <div className="absolute bottom-4 right-4 bg-purple-950/80 backdrop-blur-sm p-2 rounded-lg border border-purple-700 shadow-md">
                ...
                <div className="flex justify-center mt-2 gap-2">
                  ...
                  <span className="flex items-center text-xs text-white">{Math.round(zoomLevel * 100)}%</span>
                  <Button
                    variant="outline"
                    size="icon"
        ...
        <div className="border-t border-purple-800" style={{ height: `${100 - gridWorldHeight}%` }}>
          <GlobalKnowledgeGraph agents={agents} onSelectNode={onSelectKnowledgeNode} onShowAbout={onShowAbout} />
        </div>
      </div>
    </div>
  )
