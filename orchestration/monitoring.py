...
from .errors import OrchestrationError, categorize_error
...
class OrchestrationMetrics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        failure_rate = (
            self.failed_executions / max(1, self.total_executions)
        )
                ...
                if success:
                    ...
                    if total_retry_attempts > 0:
                        self._metrics.retry_success_rate = min(
                            1.0, 
                            self._metrics.successful_executions / total_retry_attempts
                        )
