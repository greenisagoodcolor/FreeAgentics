...
def validate_memory_optimization():
    ...
    try:
        ...
        test_matrices = [np.random.rand(500, 500).astype(np.float32) for _ in range(10)]
        ...
        print(f"   Memory range: {stats['actual_memory_mb']['min']:.3f} - {stats['actual_memory_mb']['max']:.3f} MB")
