...
class ContainerSecurityAuditor:
    ...
    def add_finding(self, category: str, severity: str, title: str, description: str, 
                   recommendation: str = "", details: Dict[str, Any] = None):
    ...
    def _audit_single_dockerfile(self, dockerfile_path: str):
        ...
        try:
            ...
            has_cleanup = any(
                'rm -rf /var/lib/apt/lists/*' in line or 
                'apt-get clean' in line or
                'apk del' in line
                for line in lines
            )
                ...
                for pattern in secret_patterns:
                    if re.search(pattern, line) and not line.strip().startswith('#'):
                        self.add_finding(
                            ...
                            {"dockerfile": dockerfile_path, "line": i, "content": line.strip()[:50] + "..."}
                        )
                        break
                ...
                if line and not line.startswith('#'):
                    if '=' in line:
                        ...
                        if any(sensitive in var_name.upper() for sensitive in sensitive_vars):
                            if var_value in ['', 'changeme', 'password', 'secret', '123456', 'admin']:
                                self.add_finding(
                                    "Secrets Management",
                                    "critical",
