...
from knowledge_graph.graph_engine import KnowledgeGraph, KnowledgeNode, NodeType, EdgeType
...
class FullPipelineDemo:
    ...
    async def demonstrate_gmn_parser(self):
        ...
        logger.info(f"✅ Parsed GMN graph with {len(gmn_graph.nodes)} nodes and {len(gmn_graph.edges)} edges")
    ...
    async def demonstrate_full_pipeline(self, gmn_graph, pymdp_model):
        ...
        user_prompt = "Create an explorer agent that can navigate a 3x3 grid to reach the bottom-right corner"
        ...
        llm_response = await self.llm_provider.generate(
            prompt=f"Convert to GMN: {user_prompt}",
            max_tokens=500
        )
        ...
        for step in range(3):
            ...
            logger.info(f"   Step {step + 1}: Agent belief entropy = {-np.sum(current_belief['qs'][0] * np.log(current_belief['qs'][0] + 1e-10)):.3f}")
            ...
            exploration_node = KnowledgeNode(
                ...
                properties={
                    ...
                    "belief_entropy": float(-np.sum(current_belief['qs'][0] * np.log(current_belief['qs'][0] + 1e-10)))
                }
            )
