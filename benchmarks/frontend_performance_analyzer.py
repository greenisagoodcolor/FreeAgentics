    ...
    def run_lighthouse_ci(self) -> Optional[LighthouseMetrics]:
        ...
        try:
            ...
            result = subprocess.run(
                ["npx", "lhci", "--version"], cwd=self.web_dir, capture_output=True, text=True
            )

            if result.returncode != 0:
                logger.warning("Lighthouse CI not available, installing...")
                subprocess.run(
                    ["npm", "install", "--save-dev", "@lhci/cli"], cwd=self.web_dir, check=True
                )

            # Run Lighthouse audit
            result = subprocess.run(
                ["npx", "lhci", "autorun"], cwd=self.web_dir, capture_output=True, text=True
            )
