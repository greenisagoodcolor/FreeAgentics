...
class DatabaseBenchmarks:
    ...
    def measure_query(self, query_type: str):
        ...
        result = QueryResult(
            query_type=query_type, execution_time=0, rows_affected=0, memory_usage=0
        )
    ...
    def test_bulk_insert(self, benchmark, test_db):
        ...
        def run_insert():
            ...
            cursor.executemany("INSERT INTO agents (name, state) VALUES (?, ?)", data)
            test_db.commit()
    ...
    def test_update_query(self, benchmark, test_db):
        ...
        def run_update():
            cursor = test_db.cursor()
            cursor.execute("UPDATE agents SET state = 'inactive' WHERE state = 'idle'")
            test_db.commit()
            return cursor.rowcount
    ...
    def test_indexed_vs_non_indexed(self, benchmark, test_db):
        ...
        def query_without_index():
            cursor.execute("SELECT * FROM messages WHERE content LIKE 'Message 5%'")
    ...
    def test_transaction_performance(self, benchmark, test_db):
        ...
        benchmark.pedantic(run_with_transaction, rounds=5)
        benchmark.pedantic(run_without_transaction, rounds=5)

        print("\nTransaction performance:")
        print("  With transaction: 100 inserts")
        print("  Without transaction: 100 inserts")
