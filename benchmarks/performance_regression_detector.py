...
class PerformanceRegressionDetector:
    ...
    def get_metric_history(self, metric_name: str, category: str = None) -> List[Tuple[datetime, float]]:
        ...
        for entry in self.performance_history:
            ...
            if isinstance(metrics, list):
                ...
                if key in metrics:
                    value = metrics[key].get('value') if isinstance(metrics[key], dict) else metrics[key]
    ...
    def _generate_recommendations(self, metric_name: str, category: str, 
                                regression_percent: float, trend: PerformanceTrend) -> List[str]:
        ...
        if trend.trend_direction == "degrading":
            recommendations.append("Performance degrading trend detected - investigate recent changes")
        ...
        if regression_percent > 0.5:  # 50%+ regression
            recommendations.append("URGENT: Severe performance regression requires immediate attention")
    ...
    def _add_alert_to_report(self, report: List[str], alert: RegressionAlert):
        ...
        report.append(f"Change: {alert.regression_percent:+.1f}% (confidence: {alert.confidence:.1%})")
...
def main():
    ...
    parser.add_argument("--results-file", type=str, required=True,
                       help="Performance results JSON file")
