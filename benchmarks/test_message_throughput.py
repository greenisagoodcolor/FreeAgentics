...
class MessageThroughputBenchmarks:
    ...
    def test_concurrent_message_routing(self, benchmark, router, sample_messages):
        ...
        for i in range(10):
            router.register_handler(
                f"agent-{i}",
                lambda msg: time.sleep(0.0001)  # Simulate work
            )
    ...
    def test_message_batching_performance(self, benchmark, router, sample_messages):
        ...
        for batch_size, throughput in results.items():
            ...
            print(f"  Batch size {batch_size}: {throughput:.1f} msg/s "
    ...
    def test_routing_algorithm_performance(self, benchmark):
        ...
        for i in range(num_messages):
            ...
            recipients = linear_router.route(topic)
        ...
        for i in range(num_messages):
            topic = f"topic-{i % num_topics}"
            recipients = hash_router.route(topic)
        ...
        print(f"  Hash lookup: {hash_time * 1000:.1f}ms "
    ...
    def test_queue_performance(self, benchmark):
        ...
        print(f"  Deque: {deque_time * 1000:.1f}ms "
              f"({num_messages / deque_time:.1f} ops/sec)")
        print(f"  List: {list_time * 1000:.1f}ms "
              f"({num_messages / list_time:.1f} ops/sec)")
