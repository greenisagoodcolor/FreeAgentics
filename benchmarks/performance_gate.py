...
class PerformanceGate:
    ...
    def check(self, current_file: str) -> GateResult:
        ...
        try:
            ...
            return GateResult(
                passed=False, message=f"Failed to load current metrics: {e}", details={}
            )
        ...
        if "agent_spawning" in current_metrics:
            ...
            if current_spawn > self.AGENT_SPAWN_TARGET_MS:
                failures.append(
                    f"Agent spawn P95 ({current_spawn:.1f}ms) > target ({self.AGENT_SPAWN_TARGET_MS}ms)"
                )
            ...
            if baseline_spawn > 0:
                ...
                if regression > self.REGRESSION_THRESHOLD:
                    failures.append(
                        f"Agent spawn regressed {regression*100:.1f}% (baseline: {baseline_spawn:.1f}ms → current: {current_spawn:.1f}ms)"
                    )
        ...
        if "memory_usage" in current_metrics:
            ...
            if current_memory > self.MEMORY_BUDGET_MB:
                failures.append(
                    f"Memory per agent ({current_memory:.1f}MB) > budget ({self.MEMORY_BUDGET_MB}MB)"
                )
            ...
            if baseline_memory > 0:
                ...
                if regression > self.REGRESSION_THRESHOLD:
                    failures.append(
                        f"Memory usage regressed {regression*100:.1f}% (baseline: {baseline_memory:.1f}MB → current: {current_memory:.1f}MB)"
                    )
        ...
        if "pymdp_inference" in current_metrics:
            ...
            if baseline_pymdp > 0:
                ...
                if regression > self.REGRESSION_THRESHOLD:
                    failures.append(
                        f"PyMDP inference regressed {regression*100:.1f}% (baseline: {baseline_pymdp:.1f}ms → current: {current_pymdp:.1f}ms)"
                    )
        ...
        message = (
            ...
            if passed
            else f"❌ {len(failures)} performance gate(s) failed"
        )
...
def main():
    ...
    if result.details.get("checks"):
        ...
        for name, check in result.details["checks"].items():
            ...
            print(
                f"  {status} {name}: {check['current']:.1f} (baseline: {check.get('baseline', 'N/A')})"
            )
