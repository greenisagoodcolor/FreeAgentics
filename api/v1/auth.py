...
from observability.honeycomb_auth_tracing import honeycomb_auth_tracer
    ...
    async with honeycomb_auth_tracer.trace_auth_operation(
        ...
        if not user:
            ...
            if span:
                ...
                span.add_tag("auth.failure_reason", "invalid_credentials")
        ...
        if not user.is_active:
            ...
            if span:
                span.add_tag("auth.success", False)
                span.add_tag("auth.failure_reason", "account_disabled")
        ...
        if span:
            ...
            span.add_tag("user.id", user.user_id)
            span.add_tag("user.role", user.role)
