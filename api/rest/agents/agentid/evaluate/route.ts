...
export async function POST(
  ...
  try {
    ...
    newScore.metrics.collaboration.successful_interactions = Math.floor(
      newScore.metrics.collaboration.total_interactions * (Math.random() * 0.2 + 0.75)
    )
    ...
    if (!newScore.is_ready) {
      ...
      if (scores.knowledge_maturity < 0.85) {
        ...
        if (needed > 0) {
          recs.push(`Gain more experience: ${newScore.metrics.knowledge.experience_count}/1000 experiences`)
        }
