...
async function callPythonExportService(
  ...
  return new Promise((resolve, reject) => {
    const pythonScript = path.join(process.cwd(), 'scripts', 'experiment_service.py')
      ...
      if (result.success) {
        return NextResponse.json({
          ...
          pagination: {
            ...
            limit: query.limit,
            offset: query.offset,
            hasMore: (query.offset + query.limit) < (result.total || result.data.length)
          }
        })
      } else {
    ...
    const mockExports = [
      {
          ...
          description: 'Mock data returned due to backend service unavailability',
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const result = await callPythonExportService('export_experiment', exportParams)
    ...
    return NextResponse.json({
      ...
      description: exportRequest.description || '',
      status: 'processing',
        message: 'Export job queued. Backend service will process when available.',
