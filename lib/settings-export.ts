 ...
 */
export async function prepareSettingsForExport(settings: LLMSettings, includeApiKeys = false): Promise<LLMSettings> {
  ...
  if (includeApiKeys && settings.apiKeySessionId) {
    try {
      ...
      const apiKey = await getApiKey(settings.provider, settings.apiKeySessionId)
      if (apiKey) {
