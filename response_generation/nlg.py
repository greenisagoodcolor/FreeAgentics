...
Explain this decision in simple, clear terms while mentioning that it used Active Inference principles. Keep it under 150 words.""",
    ...
    def _build_prompt(
        ...
        try:
            ...
            uncertainty_text = (
                ", ".join(uncertainty_areas) if uncertainty_areas else "None identified"
            )
    ...
    def _update_avg_time(self, time_ms: float) -> None:
        ...
        if current_avg == 0.0:
            ...
            alpha = 0.1
            self._metrics["avg_enhancement_time_ms"] = (alpha * time_ms) + (
                (1 - alpha) * current_avg
            )
...
class TemplateBasedGenerator(NaturalLanguageGenerator):
    ...
    async def enhance_message(
        ...
        try:
            ...
            rationale = (
                response_data.action_explanation.rationale or "This decision minimizes uncertainty."
            )
