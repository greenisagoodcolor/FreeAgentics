        ...
        if hasattr(inference_result, "metadata") and inference_result.metadata:
            if "policy_precision" in inference_result.metadata:
                decision_factors.append(
                    f"Policy precision: {inference_result.metadata['policy_precision']}"
                )
            ...
            if "planning_horizon" in inference_result.metadata:
                decision_factors.append(
                    f"Planning horizon: {inference_result.metadata['planning_horizon']}"
                )
        ...
        if states:
            try:
                ...
                if isinstance(states, list) and len(states) > 0:
                    ...
                    belief_distribution = {f"State {i}": prob for i, prob in enumerate(state_probs)}

                    # Identify uncertainty areas (states with significant probability)
                    threshold = 0.1  # 10% threshold for significant probability
                    uncertainty_areas = [
                        f"State {i}" for i, prob in enumerate(state_probs) if prob > threshold
                    ]
    ...
    def _format_confidence_rating(
        ...
        if hasattr(inference_result, "metadata") and inference_result.metadata:
            if "policy_precision" in inference_result.metadata:
                ...
                factors["policy_precision"] = min(
                    1.0, precision / 20.0
                )  # Normalize assuming max ~20
            ...
            try:
                ...
                if isinstance(states, list) and len(states) > 0:
                    ...
                    if isinstance(state_probs, list):
                        ...
                        factors["belief_certainty"] = (
                            1.0 - (entropy / max_entropy) if max_entropy > 0 else 0.5
                        )
            except Exception as e:
