...
class BusinessValueCalculationEngine:
    ...
    def calculate_business_value(
        ...
        try:
            ...
            metrics.methodology_notes["synergy"] = (
                "Coalition value exceeds sum of individual agent values through capability complementarity"
            )
    ...
    def _calculate_innovation_potential(
        ...
        innovation_score = min(1.0, len(all_capabilities) / 10.0)  # Assume 10+ is highly innovative
    ...
    def _calculate_confidence(
        ...
        if formation_result.success:
            ...
            confidence_factors.append(0.1)  # Low confidence for failed formations

        # Sample size factor
        sample_factor = min(1.0, len(agent_profiles) / 3.0)  # 3+ agents for good confidence
