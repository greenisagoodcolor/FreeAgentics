...
export async function findAvailablePort(startPort: number, portRange: number[] = [3000, 3001, 3002]): Promise<number> {
...
export async function killProcessOnPort(port: number): Promise<void> {
  try {
    ...
    console.log(`No processes found on port ${port} or kill failed:`, error instanceof Error ? error.message : String(error));
  }
}
...
export async function startServer(options: ServerStartOptions): Promise<ServerStartResult> {
  ...
  try {
    ...
    console.warn(`Failed to clean up port ${preferredPort}:`, error instanceof Error ? error.message : String(error));
  }
  
  // Find available port starting from preferred port
  const availablePort = await findAvailablePort(preferredPort, [preferredPort, preferredPort + 1, preferredPort + 2]);
