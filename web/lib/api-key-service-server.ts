...
export async function retrieveApiKey(provider: string, sessionId: string): Promise<string | null> {
  try {
    const cookieStore = await cookies();
...
export async function deleteApiKey(provider: string, sessionId: string): Promise<void> {
  try {
    const cookieStore = await cookies();
