...
const timestamp =
  process.env.TEST_TIMESTAMP || new Date().toISOString().replace(/[:.]/g, "-");
const reportDir =
  process.env.TEST_REPORT_DIR || path.join("..", "tests", "reports", timestamp);
...
switch (testType) {
  case "jest":
  case "unit":
    const jestCoverageDir =
      process.env.JEST_COVERAGE_DIR ||
      path.join(frontendReportDir, "jest", "coverage");
    command = `jest --coverage --coverageDirectory=${jestCoverageDir} ${additionalArgs}`;
    break;

  case "playwright":
  case "e2e":
    const playwrightHtmlReport =
      process.env.PLAYWRIGHT_HTML_REPORT ||
      path.join(frontendReportDir, "playwright", "report");
    const playwrightOutputDir =
      process.env.PLAYWRIGHT_TEST_OUTPUT_DIR ||
      path.join(frontendReportDir, "playwright", "test-results");

    // Set environment variables for Playwright config
    process.env.PLAYWRIGHT_HTML_REPORT = playwrightHtmlReport;
    process.env.PLAYWRIGHT_JSON_OUTPUT = path.join(
      frontendReportDir,
      "playwright",
      "results.json",
    );
    process.env.PLAYWRIGHT_JUNIT_OUTPUT = path.join(
      frontendReportDir,
      "playwright",
      "results.xml",
    );
