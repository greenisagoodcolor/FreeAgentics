...
test.describe('Agents Page', () => {
  ...
  test('can interact with agent creation flow', async ({ page }) => {
    ...
    if (await createButtons.count() > 0) {
      ...
      const modalOrForm = page.locator('[role="dialog"], form, [data-testid*="modal"], [data-testid*="form"]')
      
      if (await modalOrForm.count() > 0) {
        await expect(modalOrForm.first()).toBeVisible()
        
        // If there's a close button, test it
        const closeButton = page.locator('button[aria-label*="close"], button:has-text("Cancel"), [data-testid*="close"]')
  ...
  test('agent list renders properly', async ({ page }) => {
    ...
    const listElements = page.locator('ul, [role="list"], [data-testid*="list"], [class*="list"]')
  ...
  test('handles empty state gracefully', async ({ page }) => {
    ...
    const contentElements = page.locator('[data-testid*="agent"], [class*="agent"]')
