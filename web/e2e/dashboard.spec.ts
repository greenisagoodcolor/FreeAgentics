...
test.describe('Dashboard Functionality', () => {
  ...
  test('dashboard loads with key components', async ({ page }) => {
    ...
    const dashboardIndicators = [
      'agent', 'dashboard', 'card', 'metric', 'chart', 'overview'
    ]
    
    let foundIndicators = 0
    for (const indicator of dashboardIndicators) {
      const elements = page.locator(`[data-testid*="${indicator}"], [class*="${indicator}"], :text("${indicator}")`)
  ...
  test('dashboard data loads without errors', async ({ page }) => {
    ...
    const criticalFailures = failedRequests.filter(req => 
      !req.includes('404') // 404s might be expected for some optional resources
    )
