...
test.describe('Active Inference Real-Time Tests', () => {
  ...
  test('Active Inference dashboard loads and displays real-time data', async ({ page }) => {
  ...
  test('WebSocket connection for real-time belief updates', async ({ page }) => {
  ...
  test('Active Inference visualization renders without errors', async ({ page }) => {
    // Look for visualization components (D3.js, SVG, Canvas)
    const vizElements = page.locator('svg, canvas, [data-testid*="visualization"], [data-testid*="chart"]')
  ...
  test('real-time performance meets PRD requirements (<100ms)', async ({ page }) => {
  ...
  test('handles Active Inference errors gracefully', async ({ page }) => {
    ...
    const activeInferenceErrors = [...consoleLogs, ...errors].filter(error =>
    ...
    const criticalErrors = activeInferenceErrors.filter(error =>
      !error.includes('development') &&
      !error.includes('DevTools') &&
      !error.includes('warning')
    )
