...
function fixTestSyntax(content) {
  ...
  fixed = fixed.replace(
    /<CharacterCreator onCreate={onCreate}\s*\/\s*>/g,
    "<CharacterCreator onCreate={onCreate} />",
  );

  // Fix AgentCard tags
  fixed = fixed.replace(
    /<AgentCard agent={updatedAgent}\s*\/\s*>/g,
    "<AgentCard agent={updatedAgent} />",
  );

  // Fix MessageComponents tags
  fixed = fixed.replace(
    /<MessageComponents\.(\w+) ([^>]+)\s*\/\s*>/g,
    "<MessageComponents.$1 $2 />",
  );

  // Fix ConversationDashboard tags
  fixed = fixed.replace(
    /<ConversationDashboard ([^>]+)\s*\/\s*>/g,
    "<ConversationDashboard $1 />",
  );

  // Fix rerender calls
  fixed = fixed.replace(
    /rerender\(<ConversationDashboard ([^>]+)\s*\/\s*>\);/g,
    "rerender(<ConversationDashboard $1 />);",
  );
  fixed = fixed.replace(
    /rerender\(<ConversationDashboard ([^>]+)>\);/g,
    "rerender(<ConversationDashboard $1 />);",
  );
