...
export function StrategicPositioningDashboard() {
  ...
  useEffect(() => {
    const generateMockData = () => {
      const coalitionNames = ['AgentTech', 'DataCorp', 'InnovateCo', 'ScaleSoft', 'EdgeAI'];
      
      const mockBusinessModels: BusinessModel[] = coalitionNames.map((name, index) => ({
        ...
        uncertainty: {
          ...
          profitability: [0.5 + Math.random() * 0.1, 0.85 + Math.random() * 0.1],
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Avg Market Share</p>
                <p className="text-2xl font-bold text-gray-900">
                  {businessModels.length > 0 ? 
                    (businessModels.reduce((acc, bm) => acc + bm.marketShare, 0) / businessModels.length).toFixed(1) : 0}%
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">
                  {businessModels.length > 0 ? 
                    (businessModels.reduce((acc, bm) => acc + bm.growthRate, 0) / businessModels.length).toFixed(0) : 0}%
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">
                  {businessModels.length > 0 ? 
                    formatPercent(businessModels.reduce((acc, bm) => acc + bm.profitabilityScore, 0) / businessModels.length) : "0%"}
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-orange-600">
                  {businessModels.length > 0 ? 
                    formatPercent(businessModels.reduce((acc, bm) => acc + bm.riskScore, 0) / businessModels.length) : "0%"}
                </p>
              </div>
      ...
      <Tabs defaultValue="models" className="space-y-6">
              ...
              <Card key={model.coalitionId}>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant={model.marketShare > 25 ? "default" : "secondary"}>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-medium text-green-600">{model.growthRate.toFixed(0)}%</span>
                      </div>
                      <Progress value={model.growthRate} className="h-2" />
                      
                      <div className="flex justify-between items-center">
                        <span className="text-sm">Profitability</span>
                        <span className="font-medium">{formatPercent(model.profitabilityScore)}</span>
                      </div>
                      <Progress value={model.profitabilityScore * 100} className="h-2" />
                    </div>
                  </div>
                </CardContent>
              </Card>
        ...
        <TabsContent value="uncertainty" className="space-y-6">
          <Alert>
            ...
            <AlertDescription>
              All strategic projections include uncertainty bands and confidence intervals 
              based on Monte Carlo simulation and sensitivity analysis.
            </AlertDescription>
          </Alert>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {businessModels.map((model) => (
              <Card key={model.coalitionId}>
                ...
                <CardContent className="space-y-4">
                  <div>
                    <h5 className="font-semibold mb-3">Market Share Confidence Bands</h5>
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span>Lower Bound</span>
                        <span>{model.uncertainty.marketShare[0].toFixed(1)}%</span>
                      </div>
                      <div className="relative h-6 bg-gray-200 rounded">
                        ...
                        <div 
                          ...
                          style={{ left: `${Math.min(model.marketShare, 50)}%` }}
                        />
                      </div>
                      <div className="flex justify-between text-sm">
                        <span>Upper Bound</span>
                        <span>{model.uncertainty.marketShare[1].toFixed(1)}%</span>
                      </div>
                    </div>
                  </div>

                  <div className="bg-blue-50 p-3 rounded-lg">
                    <h6 className="font-semibold text-blue-900 mb-2">Risk Assessment</h6>
                    <div className="flex justify-between">
                      <span className="text-blue-700">Risk Score</span>
                      <span className="font-bold text-blue-900">
                        {formatPercent(model.riskScore)} 
                        (±{formatPercent((model.uncertainty.risk[1] - model.uncertainty.risk[0]) / 2)})
                      </span>
                    </div>
                  </div>
                </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-sm text-gray-600 mt-2">
            Export includes confidence intervals, uncertainty bands, and statistical significance
          </p>
        </CardContent>
      </Card>
    </div>
