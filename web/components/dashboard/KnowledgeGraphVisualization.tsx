...
const KnowledgeGraphVisualization: React.FC = () => {
  ...
  const selectedAgentId = useAppSelector(state => state.agents.selectedAgentId);
  ...
  const processGraphData = useCallback(() => {
    ...
    return { nodes, links };
  }, [knowledgeGraph, confidenceThreshold, dimensions]);
  ...
  useEffect(() => {
    ...
    nodeSelection.append('circle')
      ...
      .attr('stroke-width', 3)
      .style('filter', d => selectedNode?.id === d.id ? 'drop-shadow(0 0 10px rgba(255,255,255,0.8))' : 'none');
  ...
  }, [processGraphData, dimensions, selectedNode, selectedAgentId]);
      ...
      <div className="flex items-center justify-between p-4 border-b border-[var(--bg-tertiary)]">
        <div>
          ...
          <p className="text-sm text-[var(--text-secondary)]">
            {Object.keys(knowledgeGraph.nodes).length} nodes, {Object.keys(knowledgeGraph.edges).length} edges
          </p>
        </div>
      ...
      <div className="flex-1 flex">
        ...
        <div ref={containerRef} className="flex-1 relative">
          <svg
            ...
            className="border border-[var(--bg-tertiary)] bg-[var(--bg-secondary)]"
          />
            ...
            <CardContent className="space-y-2">
              <div className="flex items-center gap-4 text-sm">
                <span className="text-[var(--text-secondary)]">
                  Confidence: <span className="font-mono">{selectedNode.confidence.toFixed(3)}</span>
                </span>
                <span className="text-[var(--text-secondary)]">
                  Agents: <span className="font-mono">{selectedNode.agents.length}</span>
                </span>
              </div>
