...
export const CoalitionGeographicViz: React.FC<CoalitionGeographicVizProps> = ({
  ...
  const [viewMode, setViewMode] = useState<'geographic' | 'business' | 'temporal'>('geographic');
  const [zoomLevel, setZoomLevel] = useState(1);
  const [center, setCenter] = useState<[number, number]>([0, 0]);

  const handleCoalitionClick = useCallback((coalition: Coalition) => {
  ...
  useEffect(() => {
    ...
    coalitionData.forEach(coalition => {
      ...
      const centerLat = memberLocations.reduce((sum, loc) => sum + loc[0], 0) / memberLocations.length;
      const centerLng = memberLocations.reduce((sum, loc) => sum + loc[1], 0) / memberLocations.length;
      ...
      g.append("circle")
        ...
        .attr("r", getBusinessValueRadius(coalition.business_value?.total_value || 0))
        .attr("fill", getBusinessValueColor(coalition.business_value?.total_value || 0))
        .attr("stroke", "#333")
        .attr("stroke-width", 2)
        .style("cursor", "pointer")
        .on("click", () => handleCoalitionClick(coalition));
  ...
  const getCoalitionColor = (coalition: Coalition): string => {
    ...
    const hash = coalition.coalition_id.split('').reduce((acc, char) => 
          ...
          <div className="flex gap-4">
            ...
            {selectedCoalition && (
              <div className="w-80 space-y-4">
                <Card>
                  <CardHeader>
                    <CardTitle className="text-lg">{selectedCoalition.name}</CardTitle>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div>
                      <h4 className="font-semibold mb-2">Members ({selectedCoalition.members.length})</h4>
                      <div className="space-y-1">
                        {selectedCoalition.members.map(member => (
                          <div key={member.agent_id} className="flex justify-between text-sm">
                    ...
                    {showBusinessMetrics && selectedCoalition.business_value && (
                      <div>
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between text-sm">
                            ...
                            <span className="font-semibold">
                              {formatBusinessValue(selectedCoalition.business_value.total_value)}
                            </span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span>Synergy:</span>
                            <span>{formatBusinessValue(selectedCoalition.business_value.synergy_score)}</span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span>Risk Reduction:</span>
                            <span>{formatBusinessValue(selectedCoalition.business_value.risk_reduction)}</span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span>Market Position:</span>
                            <span>{formatBusinessValue(selectedCoalition.business_value.market_positioning)}</span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span>Sustainability:</span>
                            <span>{formatBusinessValue(selectedCoalition.business_value.sustainability_score)}</span>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="text-xs text-gray-500">
                      Formed: {new Date(selectedCoalition.formation_timestamp).toLocaleDateString()}
                    </div>
                  </CardContent>
                </Card>
              </div>
