  ...
  return (
    <div className="flex flex-wrap gap-2 mt-2 text-xs text-muted-foreground">
      ...
      {metadata.performanceMetrics?.tokens && (
        <div className="flex items-center gap-1">
          <span>Tokens: {metadata.performanceMetrics.tokens.input}/{metadata.performanceMetrics.tokens.output}</span>
        </div>
      )}
...
export const RegularMessage = memo<MessageComponentProps>(({
  ...
  return (
    <div className={cn("group flex gap-3 hover:bg-muted/20 transition-colors", className)}>
      {/* Avatar */}
      <Avatar className="w-8 h-8 flex-shrink-0">
        ...
        {message.metadata?.attachments && message.metadata.attachments.length > 0 && (
          <div className="mb-2">
            {message.metadata.attachments.map((attachment, index) => (
              <div key={index} className="text-xs text-muted-foreground border rounded p-2 mb-1">
