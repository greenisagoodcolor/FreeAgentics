    ...
    if (file) {
      ...
      setTimeout(() => {
        ...
        const mockMetadata: ImportMetadata = {
          ...
          description: "Experiment state export containing agents, conversations, and knowledge graphs.",
          version: "1.0.0",
          createdAt: new Date().toISOString(),
          createdBy: "user@example.com",
          components: ["Agents", "Conversations", "Knowledge Graphs", "Parameters"],
          statistics: {
        ...
        if (newProgress >= 100) {
          ...
          setTimeout(() => {
            if (Math.random() > 0.1) { // 90% success rate
              ...
              setErrorMessage("Failed to validate experiment data. The file may be corrupted or incompatible.");
              setStep('error');
            }
          }, 500);
          return 100;
  ...
  const renderUploadStep = () => (
    <>
      <DialogHeader>
        ...
        <DialogDescription>
          Import a previously exported experiment state to recreate your research environment.
        </DialogDescription>
      </DialogHeader>
      ...
      <div className="py-4 space-y-6">
        <div className="flex items-start space-x-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {(selectedFile?.size && (selectedFile.size / (1024 * 1024)).toFixed(2)) || 0} MB • {importMetadata?.createdAt ? new Date(importMetadata.createdAt).toLocaleDateString() : 'Unknown date'}
            </p>
          </div>
        </div>
          ...
          <CardContent className="text-sm">
            <div className="space-y-3">
              <div className="grid grid-cols-3 gap-1">
                ...
                <span className="font-medium col-span-2">{importMetadata?.name}</span>
              </div>
              {importMetadata?.description && (
                <div className="grid grid-cols-3 gap-1">
                  <span className="text-muted-foreground">Description:</span>
                  <span className="col-span-2">{importMetadata.description}</span>
                </div>
              )}
              <div className="grid grid-cols-3 gap-1">
              ...
              <div className="grid grid-cols-3 gap-1">
                ...
                <span className="col-span-2">{importMetadata?.components.join(', ')}</span>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-3 gap-4">
          <Card className="p-4 flex flex-col items-center">
            ...
            <div className="text-2xl font-bold">{importMetadata?.statistics.totalAgents}</div>
            <div className="text-xs text-muted-foreground">Agents</div>
          </Card>
          <Card className="p-4 flex flex-col items-center">
            <MessageSquare className="h-6 w-6 text-green-500 mb-2" />
            <div className="text-2xl font-bold">{importMetadata?.statistics.totalConversations}</div>
            <div className="text-xs text-muted-foreground">Conversations</div>
          </Card>
          <Card className="p-4 flex flex-col items-center">
            <Database className="h-6 w-6 text-purple-500 mb-2" />
            <div className="text-2xl font-bold">{importMetadata?.statistics.totalKnowledgeNodes}</div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-md p-3 flex items-start space-x-3">
          ...
          <div className="text-sm text-amber-800">
            ...
            <p className="mt-1">
              Importing this experiment will create new agents, conversations, and knowledge graphs in your environment.
        ...
        <Card>
          ...
          <CardContent className="text-sm space-y-3">
            <div className="grid grid-cols-2 gap-1">
              ...
              <span className="font-medium">{importMetadata?.statistics.totalAgents} imported</span>
            </div>
            <div className="grid grid-cols-2 gap-1">
              <span className="text-muted-foreground">Conversations:</span>
              <span className="font-medium">{importMetadata?.statistics.totalConversations} imported</span>
            </div>
            <div className="grid grid-cols-2 gap-1">
              <span className="text-muted-foreground">Knowledge Nodes:</span>
              <span className="font-medium">{importMetadata?.statistics.totalKnowledgeNodes} imported</span>
            </div>
            <div className="grid grid-cols-2 gap-1">
