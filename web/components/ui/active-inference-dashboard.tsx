...
import { BeliefStateVisualization, BeliefStateData } from "./belief-state-visualization";
import { FreeEnergyVisualization, FreeEnergyData } from "./free-energy-visualization";
...
export function ActiveInferenceDashboard({
  ...
  onStateChange,
  isRealTime = true,
  stateLabels = ["Explore", "Navigate", "Rest", "Communicate", "Learn", "Plan", "Execute", "Observe"],
  ...
  const generatePrecisionData = useCallback((
    ...
    const feNormalized = freeEnergyData ? Math.max(0, 1 - freeEnergyData.variationalFreeEnergy / 5) : 0.7;
  ...
  const handleBeliefChange = useCallback((beliefData: BeliefStateData) => {
    setAgentState(prev => {
  ...
  const handleFreeEnergyChange = useCallback((freeEnergyData: FreeEnergyData) => {
    setAgentState(prev => {
      const newPrecision = generatePrecisionData(prev.beliefs, freeEnergyData);
      const newState = {
        ...prev,
      ...
      if (freeEnergyData.variationalFreeEnergy > 5.0) {
        setAlerts(prev => [...prev.filter(a => !a.includes("High free energy")), "High free energy - agent struggling"]);
      }
  ...
  useEffect(() => {
    ...
    if (agentState.freeEnergy && agentState.freeEnergy.variationalFreeEnergy > 4.5) {
      status = "error";
    } else if (agentState.precision && agentState.precision.performance.convergence < 0.3) {
      status = "learning";
  ...
  }, [agentState.freeEnergy, agentState.precision, isGlobalPlay, agentState.status]);
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <Badge 
                variant={agentState.status === "active" ? "default" : 
                        agentState.status === "learning" ? "secondary" :
                        agentState.status === "error" ? "destructive" : "outline"}
              >
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {isGlobalPlay ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-lg font-bold">
                  {agentState.beliefs ? `${(agentState.beliefs.confidence * 100).toFixed(0)}%` : "--"}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-lg font-bold">
                  {agentState.freeEnergy ? agentState.freeEnergy.variationalFreeEnergy.toFixed(2) : "--"}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-lg font-bold">
                  {agentState.precision ? agentState.precision.sensory.toFixed(1) : "--"}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-lg font-bold">
                  {agentState.precision ? `${(agentState.precision.performance.accuracy * 100).toFixed(0)}%` : "--"}
                </div>
              </div>
            </div>
          </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <Progress value={Math.min(100, (agentState.precision.sensory / 50) * 100)} />
                    </div>
                    ...
                    <div>
                      ...
                      <Progress value={Math.min(100, (agentState.precision.policy / 30) * 100)} />
                    </div>
                    ...
                    <div>
                      ...
                      <Progress value={Math.min(100, (agentState.precision.state / 10) * 100)} />
                    </div>
                  </div>
                </div>
              </CardContent>
              ...
              {agentState.precision ? (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <Card>
                      <CardHeader className="pb-3">
                        <CardTitle className="text-lg">Sensory Precision (γ)</CardTitle>
                      </CardHeader>
                      <CardContent>
                        ...
                        <Progress 
                          value={Math.min(100, (agentState.precision.sensory / 50) * 100)} 
                          className="mt-3"
                        />
                      </CardContent>
                    </Card>
                    ...
                    <Card>
                      <CardHeader className="pb-3">
                        <CardTitle className="text-lg">Policy Precision (β)</CardTitle>
                      </CardHeader>
                      <CardContent>
                        ...
                        <Progress 
                          value={Math.min(100, (agentState.precision.policy / 30) * 100)} 
                          className="mt-3"
                        />
                      </CardContent>
                    </Card>
                    ...
                    <Card>
                      <CardHeader className="pb-3">
                        <CardTitle className="text-lg">State Precision (α)</CardTitle>
                      </CardHeader>
                      <CardContent>
                        ...
                        <Progress 
                          value={Math.min(100, (agentState.precision.state / 10) * 100)} 
                          className="mt-3"
                        />
                      </CardContent>
                    </Card>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-2xl font-bold">
                            {(agentState.precision.performance.accuracy * 100).toFixed(1)}%
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-2xl font-bold">
                            {(agentState.precision.performance.stability * 100).toFixed(1)}%
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-2xl font-bold">
                            {(agentState.precision.performance.convergence * 100).toFixed(1)}%
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <h4 className="font-semibold text-blue-900 mb-2">Precision Parameter Mathematics</h4>
                    <div className="text-sm text-blue-800 space-y-1">
                      <p>• <strong>γ (Sensory)</strong>: Inverse variance of sensory noise σ⁻²</p>
                      <p>• <strong>β (Policy)</strong>: Temperature parameter in softmax policy selection</p>
                      <p>• <strong>α (State)</strong>: Precision of state transition predictions</p>
                      <p>• Higher precision → Lower uncertainty → More confident decisions</p>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="text-center text-muted-foreground py-12">
                  ...
                  <p className="text-sm">Start the agent to see precision parameters</p>
                </div>
              )}
            </CardContent>
          </Card>
