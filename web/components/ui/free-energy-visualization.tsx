...
export function FreeEnergyVisualization({
  ...
  const generateMockFreeEnergyData = useCallback(
    (prevData?: FreeEnergyData): FreeEnergyData => {
      const now = Date.now();

      // Simulate dynamic free energy with temporal correlation
      const baseAccuracy =
        2.5 + Math.sin(now / 10000) * 0.5 + (Math.random() - 0.5) * 0.3;
      const baseComplexity =
        1.2 + Math.cos(now / 15000) * 0.3 + (Math.random() - 0.5) * 0.2;
      ...
      const predictionError = Math.max(
        0.1,
        2.0 - accuracy + (Math.random() - 0.5) * 0.4,
      );
      const predictionConfidence = 1 / (1 + predictionError);

      // Precision parameters with some correlation to free energy
      const sensoryPrecision =
        16 + (3 - variationalFreeEnergy) * 2 + Math.random() * 8;
      const policyPrecision =
        12 + (3 - variationalFreeEnergy) * 1.5 + Math.random() * 6;
      const statePrecision =
        2 + (3 - variationalFreeEnergy) * 0.5 + Math.random() * 2;
  ...
  useEffect(() => {
    ...
    const visibleData = freeEnergyHistory.data.filter(
    ...
    const xScale = d3
      .scaleTime()
      .domain(
        d3.extent(visibleData, (d) => new Date(d.timestamp)) as [Date, Date],
      )
      .range([0, chartWidth]);
        ...
        <div ref={containerRef} className="w-full">
          ...
          <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
            <div className="text-sm text-red-800">
              ...
              <p className="text-xs space-y-1">
                ...
                Complexity
                <br />• <strong>Accuracy</strong>: -E<sub>q</sub>[ln p(o|s)]
                (negative log likelihood)
                <br />• <strong>Complexity</strong>: D<sub>KL</sub>[q(s)||p(s)]
                (KL divergence prior)
                <br />• <strong>G(π)</strong>: Expected Free Energy for policy π
                (planning objective)
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between mt-4 text-sm text-muted-foreground">
            <span>
              ...
              {freeEnergyHistory.data.length} data points •{timeWindow / 1000}s
              window
            </span>
            <span>
              {currentData &&
