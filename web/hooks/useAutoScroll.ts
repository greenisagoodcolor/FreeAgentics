...
export interface AutoScrollOptions {
  ...
  onScrollStateChange?: (isAutoScrollEnabled: boolean, isAtBottom: boolean) => void;
  onUserOverride?: () => void;
...
export function useAutoScroll(
  ...
  const scrollToElement = useCallback((element: HTMLElement, smooth: boolean = true) => {
    const container = containerRef.current;
    if (!container) return;
  ...
  const handleScroll = useCallback(
    debounce(() => {
      ...
      const isUserInitiated = !state.isScrolling && 
