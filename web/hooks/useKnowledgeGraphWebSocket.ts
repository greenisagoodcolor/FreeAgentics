...
export interface UseKnowledgeGraphWebSocketReturn {
  ...
  subscribe: (eventType: string, callback: (update: KnowledgeGraphUpdate) => void) => () => void;
}
...
export function useKnowledgeGraphWebSocket(
  ...
  const eventListenersRef = useRef<Map<string, Set<(update: KnowledgeGraphUpdate) => void>>>(
    new Map()
  );
