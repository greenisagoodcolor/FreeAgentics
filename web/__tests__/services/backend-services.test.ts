...
class ConversationService {
  ...
  private async processMessage(conversationId: string, message: any): Promise<void> {
...
class KnowledgeService {
  ...
  async importKnowledge(data: string, format: 'json' | 'csv' = 'json'): Promise<number> {
    let entries: any[] = [];
  ...
  getStats(): any {
    ...
    entries.forEach(entry => entry.tags.forEach((tag: string) => allTags.add(tag)));
  ...
  describe('AgentService', () => {
    ...
    describe('Agent Updates', () => {
      ...
      it('returns null when updating non-existent agent', async () => {
        const result = await agentService.updateAgent('non-existent', { name: 'Test' });
        expect(result).toBeNull();
      });
  ...
  describe('ConversationService', () => {
    ...
    describe('Conversation Listing', () => {
      it('lists conversations by recent activity', async () => {
        const conv1 = await conversationService.createConversation({ title: 'First' });
        await new Promise(resolve => setTimeout(resolve, 10));
        const conv2 = await conversationService.createConversation({ title: 'Second' });
      ...
      it('filters by status', async () => {
        const conv1 = await conversationService.createConversation({ title: 'Active' });
        const conv2 = await conversationService.createConversation({ title: 'Inactive' });
        
        await conversationService.startConversation(conv1.id);
        
        const activeConversations = await conversationService.listConversations({ status: 'active' });
  ...
  describe('KnowledgeService', () => {
    ...
    describe('Search Functionality', () => {
      ...
      it('finds entries with any of multiple tags', async () => {
        const results = await knowledgeService.getEntriesByTags(['javascript', 'database']);
