...
const mockD3 = {
  ...
  scaleLinear: jest.fn(() => ({
    ...
    nice: jest.fn(function() { return this; }),
  })),
  ...
  line: jest.fn(() => ({
    x: jest.fn(function() { return this; }),
    y: jest.fn(function() { return this; }),
  })),
};
...
class SystemManager {
  ...
  sendMessage(conversationId: string, agentId: string, content: string): Message {
  ...
  addKnowledgeNode(type: string, label: string, properties: Record<string, any>): KnowledgeNode {
  ...
  addKnowledgeEdge(sourceId: string, targetId: string, relationship: string): KnowledgeEdge {
  ...
  // Belief Management
  updateBelief(agentId: string, beliefKey: string, value: number, confidence: number = 0.8): void {
    ...
    this.eventBus.emit('belief_updated', { agentId, beliefKey, value, confidence });
  }
  ...
  private calculateCoalitionStrength(memberIds: string[]): number {
    ...
    const avgCapabilities = members.reduce((sum, agent) => 
      sum + agent.capabilities.length, 0) / members.length;
    const avgPerformance = members.reduce((sum, agent) => 
      sum + agent.performance.success_rate, 0) / members.length;
    ...
    <div data-testid="system-dashboard" className="system-dashboard">
      <header className="dashboard-header">
        ...
        <div className="system-status">
          ...
          <span data-testid="conversation-count">Conversations: {state.conversations.length}</span>
          <span data-testid="coalition-count">Coalitions: {state.coalitions.length}</span>
          <span data-testid="knowledge-nodes">Knowledge Nodes: {state.knowledgeGraph.nodes.length}</span>
        </div>
      </header>
      ...
      <div className="dashboard-grid">
        <div className="dashboard-section">
          ...
          <div data-testid="system-metrics">
            ...
            <div>Error Rate: {(state.metrics.errors.rate * 100).toFixed(2)}%</div>
          </div>
        </div>
      ...
      {selectedAgent && (
        <div data-testid="agent-details" className="agent-details">
          <h4>Selected Agent: {state.agents.find(a => a.id === selectedAgent)?.name}</h4>
        </div>
      )}

      {selectedConversation && (
        <div data-testid="conversation-details" className="conversation-details">
          <h4>Selected Conversation: {state.conversations.find(c => c.id === selectedConversation)?.title}</h4>
        </div>
      )}
    </div>
  );
...
describe('Complete System Integration Tests', () => {
  ...
  describe('Agent Management', () => {
    ...
    it('handles non-existent agent operations', () => {
      const updated = systemManager.updateAgent('nonexistent', { name: 'Test' });
  ...
  describe('Conversation Management', () => {
    it('creates conversations with participants', () => {
      ...
      const conversation = systemManager.createConversation(
        'Test Chat',
        [agent1.id, agent2.id]
      );
  ...
  describe('Belief Management', () => {
    it('creates new belief states', () => {
      ...
      expect(agentBelief?.uncertainty).toBe(0.1);
    });
  ...
  describe('Coalition Management', () => {
    it('forms coalitions with members', () => {
      const agent1 = systemManager.createAgent({ name: 'Agent 1', capabilities: ['analysis'] });
      const agent2 = systemManager.createAgent({ name: 'Agent 2', capabilities: ['communication'] });
  ...
  describe('System Metrics', () => {
    ...
    it('tracks usage metrics', () => {
      ...
      expect(metrics.usage.active_users).toBe(2);
  ...
  describe('DataStore', () => {
    it('manages cache with TTL', () => {
      ...
      store.setCache('key2', 'value2', 0); // Immediate expiry
  ...
  describe('SystemDashboard Component', () => {
    it('renders system dashboard', () => {
      ...
      expect(screen.getByTestId('coalition-count')).toHaveTextContent('Coalitions: 0');
    });
