...
import { AgentList, AgentBeliefVisualizer, CharacterCreator } from '../../__mocks__/components/stubs';
...
describe('Agent Components', () => {
  describe('AgentList', () => {
    ...
    it('handles agent selection', () => {
      ...
      render(<AgentList agents={mockAgents} onAgentSelect={onSelect} {...({} as any)} />);
      
      const firstAgent = screen.getByText('Knowledge Seeker').closest('article');
    ...
    it('shows performance indicators', () => {
      render(<AgentList agents={mockAgents} showPerformance {...({} as any)} />);
  ...
  describe('AgentCard', () => {
    ...
    it('handles card click', () => {
      ...
      render(
        <AgentCard agent={(mockAgents[0] as any)}
          onClick={onClick} {...({} as any)} />
      );
      
      // Click on the card
      const card = screen.getByText('Knowledge Seeker').closest('.cursor-pointer');
      if (card) {
    ...
    it('shows autonomy status', () => {
      ...
      const autonomyIndicator = screen.getByRole('button', { name: /autonomy/i });
  ...
  describe('AgentDashboard', () => {
    it('renders dashboard overview', () => {
      ...
      render(<AgentDashboard agents={mockAgents} onSelectAgent={onSelectAgent} selectedAgent={null} {...({} as any)} />);
    ...
    it('handles agent selection', () => {
      ...
      const agentCard = screen.getByText('Knowledge Seeker').closest('div[role="button"]');
      if (agentCard) {
  ...
  describe('AgentBeliefVisualizer', () => {
    it('renders belief visualization', () => {
      render(<AgentBeliefVisualizer agent={(mockAgents[0] as any)} {...({} as any)} />);
    ...
    it('shows belief evolution over time', async (): Promise<void> => {
      const beliefHistory = [
        { timestamp: new Date(Date.now() - 3600000), beliefs: { exploration: 0.5 } },
        { timestamp: new Date(Date.now() - 1800000), beliefs: { exploration: 0.7 } },
        { timestamp: new Date(), beliefs: { exploration: 0.8 } },
      ];
      
      render(
        <AgentBeliefVisualizer agent={(mockAgents[0] as any)} 
          history={beliefHistory} {...({} as any)} />
      );
    ...
    it('highlights belief changes', () => {
      ...
      render(
        <AgentBeliefVisualizer agent={(mockAgents[0] as any)}
          previousBeliefs={previousBeliefs} {...({} as any)} />
      );
    ...
    it('supports interactive belief adjustment', () => {
      ...
      render(
        <AgentBeliefVisualizer agent={(mockAgents[0] as any)}
          editable
          onBeliefChange={onBeliefChange} {...({} as any)} />
      );
  ...
  describe('CharacterCreator', () => {
    ...
    it('supports agent templates', () => {
      ...
      expect(screen.getByLabelText(/agent name/i)).toHaveValue('Research Agent');
  ...
  describe('Agent Performance Tracking', () => {
    it('tracks real-time performance metrics', async (): Promise<void> => {
      const { rerender } = render(<AgentCard agent={(mockAgents[0] as any)} {...({} as any)} />);
    ...
    it('shows performance trends', () => {
      ...
      render(
        <AgentDashboard agents={mockAgents}
          onSelectAgent={onSelectAgent}
          selectedAgent={(mockAgents[0] as any)} {...({} as any)} />
      );
