...
class LogPatternMatcher:
    ...
    def _load_builtin_patterns(self):
        ...
        self.add_pattern(
            ...
            LogLevel.INFO,
            extractor=lambda msg: {"duration_ms": float(re.search(r"(\d+(?:\.\d+)?)\s*ms", msg).group(1))}
        )
        ...
        self.add_pattern(
            ...
            extractor=lambda msg: {
                "memory_mb": self._parse_memory(msg)
            }
        )
        ...
        self.add_pattern(
            ...
            LogCategory.PERFORMANCE,
            LogLevel.WARNING if "90" in r"CPU usage: (?P<cpu_percent>\d+(?:\.\d+)?)%" else LogLevel.INFO
        )
    ...
    def classify_log(self, raw_message: str, default_level: LogLevel = LogLevel.INFO) -> Tuple[LogCategory, LogLevel, Dict[str, Any]]:
...
class AnomalyDetector:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.error_rate_history: Dict[str, deque] = defaultdict(lambda: deque(maxlen=self.window_size))
        self.response_time_history: Dict[str, deque] = defaultdict(lambda: deque(maxlen=self.window_size))
        self.log_volume_history: Dict[str, deque] = defaultdict(lambda: deque(maxlen=self.window_size))
        self.component_health_history: Dict[str, deque] = defaultdict(lambda: deque(maxlen=self.window_size))

    def detect_anomalies(self, logs: List[LogEntry], time_window_minutes: int = 5) -> List[AnomalyDetectionResult]:
    ...
    def _detect_error_rate_anomalies(self, component: str, logs: List[LogEntry]) -> List[AnomalyDetectionResult]:
    ...
    def _detect_response_time_anomalies(self, component: str, logs: List[LogEntry]) -> List[AnomalyDetectionResult]:
    ...
    def _detect_log_volume_anomalies(self, component: str, logs: List[LogEntry]) -> List[AnomalyDetectionResult]:
        ...
        if baseline_std > 0:
            ...
            if z_score > self.z_score_threshold:
                anomaly_type = "log_volume_spike" if current_volume > baseline_mean else "log_volume_drop"
                severity = "high" if z_score > 4 else "medium"
    ...
    def _detect_pattern_anomalies(self, component: str, logs: List[LogEntry]) -> List[AnomalyDetectionResult]:
            ...
            for error_type, count in error_counter.items():
                if error_type and count >= 3:  # At least 3 occurrences
                    anomalies.append(AnomalyDetectionResult(
                        ...
                        affected_logs=[log for log in error_logs if log.error_type == error_type],
                        anomaly_score=count,
                        actual_value=count,
    ...
    def _detect_correlation_anomalies(self, logs_by_component: Dict[str, List[LogEntry]]) -> List[AnomalyDetectionResult]:
        ...
        high_error_components = [
            comp for comp, rate in components_with_errors.items() 
        ...
        if len(high_error_components) >= 2:
            ...
            for comp in high_error_components:
                total_affected_logs.extend([
                    log for log in logs_by_component[comp] 
...
class AdvancedLogAnalytics:
    ...
    def ingest_log(
        ...
        **kwargs
        ...
        category, detected_level, extracted_data = self.pattern_matcher.classify_log(raw_message, log_level)
        
        # Use detected level if it's more severe
        if detected_level.value in ["ERROR", "CRITICAL"] and log_level.value in ["DEBUG", "INFO", "WARNING"]:
            ...
            try:
                ...
                if analysis["anomalies"]:
                    ...
                    for anomaly in analysis["anomalies"][:3]:  # Log top 3
                        logger.warning(f"Anomaly: {anomaly['description']} (score: {anomaly['anomaly_score']:.2f})")
    ...
    def _generate_analysis_summary(
        ...
        return {
            ...
            "most_active_component": max(Counter(log.component for log in logs).items(), key=lambda x: x[1])[0] if logs else "unknown",
    ...
    def _calculate_log_volume_trend(self, logs: List[LogEntry]) -> str:
        ...
        first_half_rate = len(first_half) / ((first_half[-1].timestamp - first_half[0].timestamp) / 60) if len(first_half) > 1 else 0
        second_half_rate = len(second_half) / ((second_half[-1].timestamp - second_half[0].timestamp) / 60) if len(second_half) > 1 else 0
    ...
    def _analyze_performance_patterns(self, logs: List[LogEntry]) -> Dict[str, Any]:
        ...
        return {
            ...
            "slowest_components": dict(Counter([
                log.component for log in performance_logs
    ...
    def _analyze_component_health(self, logs: List[LogEntry]) -> Dict[str, Any]:
        ...
        for component in set(log.component for log in logs):
            ...
            error_logs = [log for log in component_logs if log.level in [LogLevel.ERROR, LogLevel.CRITICAL]]
    ...
    def _identify_top_issues(self, logs: List[LogEntry]) -> List[Dict[str, Any]]:
        ...
        slow_logs = [log for log in logs if log.duration_ms and log.duration_ms > 1000]  # > 1 second
        if slow_logs:
