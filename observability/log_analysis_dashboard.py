...
class LogAnalysisEngine:
    ...
    async def analyze_agent_logs(
        ...
        logs = await self.aggregator.query_logs(
            start_time=start_time, end_time=end_time, agent_id=agent_id, limit=10000
        )
    ...
    async def _query_logs_for_analysis(
        self, start_time: datetime, end_time: datetime, sources: Optional[List[LogSource]]
    ) -> List[LogEntry]:
        ...
        if sources:
            ...
            for source in sources:
                source_logs = await self.aggregator.query_logs(
                    start_time=start_time, end_time=end_time, source=source, limit=5000
                )
    ...
    def _analyze_common_actions(self, logs: List[LogEntry]) -> List[Dict[str, Any]]:
        ...
        for action, count in action_counts.most_common(10):
            common_actions.append(
                {"action": action, "count": count, "frequency": count / len(logs) if logs else 0}
            )
