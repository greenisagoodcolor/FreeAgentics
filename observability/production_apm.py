...
class ServiceTopology:
    ...
    def add_edge(self, source: str, target: str, operation: str):
        ...
        existing = next((e for e in self.edges if 
                        e["source"] == source and 
                        e["target"] == target and 
                        e["operation"] == operation), None)
...
class ProductionAPM:
    ...
    async def record_business_metric(
        ...
        self.traces["business_metrics"] = [
            m for m in self.traces["business_metrics"]
        ...
        return {
            ...
            "sla_status": {
                ...
                "critical_violations": len([
                    v for v in self.sla_violations
                    if v["timestamp"] > now - 3600 and v["type"] in ["error_rate", "response_time_p95"]
                ])
            }
        }
    ...
    async def _update_topology(self, trace: TransactionTrace):
        ...
        if service not in self.service_topology.nodes:
            self.service_topology.add_service(service, {
                "agent_id": trace.agent_id,
                "first_operation": trace.operation
            })
    ...
    def _analyze_topology(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "highest_error_services": sorted(
                [(name, node) for name, node in self.service_topology.nodes.items()
    ...
    def _calculate_health_score(self, analytics: Dict[str, Any]) -> float:
        ...
        p95_response = analytics.get("request_analytics", {}).get("response_time_stats", {}).get("p95", 0)
