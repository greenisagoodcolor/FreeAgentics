...
rate_limit_blocks_total = Counter(
    ...
    [
        "block_type",
        "reason",
    ],  # block_type: ip, user; reason: rate_limit, ddos, suspicious
)
...
rate_limit_config_info = Info(
    "freeagentics_rate_limit_config", "Rate limiting configuration information"
)
...
class RateLimitingMetrics:
    ...
    def update_remaining_requests(
        ...
        rate_limit_remaining_gauge.labels(endpoint=endpoint, identifier=identifier).set(
            remaining
        )
