...
async def demo_pymdp_kg_pipeline():
    ...
    try:
        ...
        print(
            f"Inference: action={high_conf_result.action}, confidence={high_conf_result.confidence}"
        )
        ...
        if kg_result:
            ...
            print(
                f"✓ Generated {len(entities)} entities, {len(relations)} relations, {len(events)} events"
            )
        ...
        print(
            f"Inference: action={uncertain_result.action}, confidence={uncertain_result.confidence}"
        )
