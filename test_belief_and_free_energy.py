    ...
    try:
        ...
        for i, obs in enumerate(observations):
            ...
            if agent.pymdp_agent and hasattr(agent.pymdp_agent, 'qs') and agent.pymdp_agent.qs is not None:
                ...
                assert abs(np.sum(beliefs) - 1.0) < 1e-6, f"Beliefs don't sum to 1: {np.sum(beliefs)}"
        ...
        if 'error' in fe_components:
            ...
            for component in required_components:
                if component in fe_components:
                    ...
                    assert isinstance(value, (int, float)), f"{component} is not numeric: {type(value)}"
            ...
            if all(comp in fe_components for comp in ['total_free_energy', 'accuracy', 'complexity']):
...
def test_action_selection_free_energy():
    ...
    try:
        ...
        efe_values = [r['expected_free_energy'] for r in action_results if r['expected_free_energy'] is not None]
        if efe_values:
