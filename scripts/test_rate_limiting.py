...
class RateLimitTester:
    ...
    async def test_endpoint_rate_limit(self, endpoint, requests_per_test=10, delay_between_requests=0.1):
        ...
        for i in range(requests_per_test):
            ...
            if status == 429:
                print(f"  Request {i+1}: Rate limited! Retry-After: {headers.get('Retry-After', 'N/A')}s")
            elif success:
    ...
    async def test_authenticated_vs_anonymous(self, endpoint, auth_token=None):
        ...
        print("\nAnonymous requests:")
        self.reset_results()
        await self.test_endpoint_rate_limit(endpoint, requests_per_test=10, delay_between_requests=0.1)
        ...
        if auth_token:
            ...
            for i in range(10):
                ...
                if status == 429:
                    ...
                    print(f"  Request {i+1}: Success. Remaining: {rate_headers.get('X-RateLimit-Remaining', 'N/A')}")
            ...
            # Compare results
            print("\nComparison:")
            print(f"  Anonymous - Successful: {anonymous_results['successful_requests']}, Rate limited: {anonymous_results['rate_limited_requests']}")
            print(f"  Authenticated - Successful: {authenticated_results['successful_requests']}, Rate limited: {authenticated_results['rate_limited_requests']}")
