...
class PipelineMetrics:
    ...
    def init_database(self):
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_pipeline_runs_branch ON pipeline_runs(branch)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_pipeline_runs_status ON pipeline_runs(status)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_pipeline_runs_date ON pipeline_runs(started_at)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_stage_metrics_pipeline ON stage_metrics(pipeline_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_quality_metrics_pipeline ON quality_metrics(pipeline_id)")
    ...
    def get_pipeline_health_score(self) -> Dict[str, Any]:
        ...
        if avg_duration > 0:
            duration_score = max(0, 100 - ((avg_duration - target_duration) / target_duration * 100))
        ...
        return {
            ...
            'most_failing_stage': failure_trends['stage_failures'][0]['stage'] if failure_trends['stage_failures'] else None
        }
...
"""
        ...
        for pipeline in data['recent_pipelines'][:5]:
            status_emoji = '✅' if pipeline['status'] == 'success' else '❌' if pipeline['status'] == 'failure' else '⏭️'
