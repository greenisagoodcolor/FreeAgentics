...
class PipelineMetrics:
        ...
        try:
            ...
            if pipeline_data.get("start_time") and pipeline_data.get("end_time"):
                start = datetime.fromisoformat(
                    pipeline_data["start_time"].replace("Z", "+00:00")
                )
                end = datetime.fromisoformat(
                    pipeline_data["end_time"].replace("Z", "+00:00")
                )
    ...
    def get_average_duration(
        self, days: int = 30, branch: str = None
    ...
    def get_pipeline_health_score(self) -> Dict[str, Any]:
        ...
        if success_rate_7d < success_rate_30d:
            trend_score = max(
                0, trend_score - ((success_rate_30d - success_rate_7d) * 2)
            )
...
"""
        ...
        for pipeline in data["recent_pipelines"][:5]:
            ...
            duration = (
                f"{pipeline['duration_minutes']}m"
                if pipeline["duration_minutes"]
                else "N/A"
            )
...
def main():
    ...
    parser.add_argument(
        "--record", type=str, help="Record pipeline data from JSON file"
    )
    parser.add_argument(
        "--report",
