...
class CoverageChecker:
    ...
    def run_coverage_collection(self, module_path: Optional[str] = None) -> Dict:
        ...
        try:
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                timeout=self.PERFORMANCE_BUDGETS["total_collection_time"],
            )
            ...
            print(
                f"⚠️  Coverage collection timed out after {self.PERFORMANCE_BUDGETS['total_collection_time']}s"
            )
        ...
        for threshold in self.MODULE_THRESHOLDS:
            ...
            if threshold.criticality == "debt":
                ...
                if threshold.improvement_target:
                    coverage_debt[module_name] = (
                        f"Current: {coverage:.1f}%, Target: {threshold.improvement_target}"
                    )
            else:
    ...
    def generate_next_actions(
        self, results: List[CoverageResult], violations: List[str]
    ) -> List[str]:
    ...
    def print_summary(self, report: CoverageReport):
        ...
        sorted_modules = sorted(
            report.module_results, key=lambda r: r.coverage_percent, reverse=True
        )
        for result in sorted_modules:
            ...
            print(
                f"   {status} {result.module:20} {result.coverage_percent:6.1f}% ({result.covered_lines}/{result.total_lines})"
            )
...
def main():
    ...
    parser.add_argument(
        "--profile",
        choices=["dev", "ci", "release", "default"],
        default="default",
        help="Coverage profile to use",
    )
    parser.add_argument("--module", help="Check specific module only")
    parser.add_argument(
        "--no-collect", action="store_true", help="Skip coverage collection, analyze existing data"
    )
    parser.add_argument(
        "--output", default="coverage-report.json", help="Output file for JSON report"
    )
    ...
    if not args.no_collect:
        ...
        if not collection_result["success"]:
            print(
                f"❌ Coverage collection failed: {collection_result.get('error', 'unknown error')}"
            )
            sys.exit(1)
    else:
