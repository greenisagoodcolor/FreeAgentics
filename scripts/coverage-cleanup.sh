...
find . -name ".coverage" -type f -exec rm -f {} \; 2>/dev/null || true
find . -name ".coverage.*" -type f -exec rm -f {} \; 2>/dev/null || true
find . -name "coverage.xml" -type f -exec rm -f {} \; 2>/dev/null || true
find . -name "coverage.json" -type f -exec rm -f {} \; 2>/dev/null || true
find . -name "coverage.lcov" -type f -exec rm -f {} \; 2>/dev/null || true
find . -name ".coverage.old" -type f -exec rm -f {} \; 2>/dev/null || true

# 2. Remove HTML coverage directories
print_status "Cleaning HTML coverage directories..."
find . -name "htmlcov" -type d -exec rm -rf {} \; 2>/dev/null || true
find . -name "htmlcov_backup" -type d -exec rm -rf {} \; 2>/dev/null || true
find . -name "coverage_html_report" -type d -exec rm -rf {} \; 2>/dev/null || true

# 3. Clean up test report directories (keep structure, remove old reports)
print_status "Cleaning old test reports..."
if [ -d "test-reports" ]; then
    find test-reports -name "*.xml" -mtime +7 -delete 2>/dev/null || true
...
find . -name ".coveragerc.old" -type f -delete 2>/dev/null || true
find . -name ".coveragerc.backup" -type f -delete 2>/dev/null || true
find . -name "pytest.ini.old" -type f -delete 2>/dev/null || true
...
find . -name "coverage-report*.txt" -type f -delete 2>/dev/null || true
find . -name "coverage-analysis*.json" -type f -delete 2>/dev/null || true
find . -name "coverage-gaps*.txt" -type f -mtime +1 -delete 2>/dev/null || true
...
if [ -d "web" ]; then
    ...
    find web -name "coverage-final.json" -type f -delete 2>/dev/null || true
fi

# 10. Remove temporary coverage files
print_status "Cleaning temporary coverage files..."
find /tmp -name ".coverage*" -user $(whoami) -delete 2>/dev/null || true
