...
class CoverageBaseline:
    ...
    def print_baseline_report(self, report: BaselineReport):
        ...
        print(f"\n📂 Module Progress:")
        print(
            f"{'Module':<20} {'Baseline':<10} {'Current':<10} {'Target':<10} {'Progress':<12} {'Velocity':<10}"
        )
        print("-" * 80)
        ...
        for baseline in sorted_baselines:
            ...
            print(
                f"{baseline.module:<20} {baseline.baseline_coverage:>8.1f}% "
        ...
        if zero_modules:
            ...
            for baseline in zero_modules:
                print(
                    f"   {baseline.module}: Target {baseline.target_coverage}% by {baseline.target_date}"
                )

        # Achievement callouts
        achieved_targets = [
            b for b in report.baselines if (b.current_coverage or 0) >= b.target_coverage
        ]
        if achieved_targets:
            print(f"\n🎉 Target Achievements ({len(achieved_targets)}):")
            for baseline in achieved_targets:
                print(
                    f"   {baseline.module}: {baseline.current_coverage:.1f}% (target: {baseline.target_coverage}%)"
                )


def main():
    parser = argparse.ArgumentParser(description="Coverage baseline establishment and tracking")
    parser.add_argument("--establish", action="store_true", help="Establish new coverage baseline")
    parser.add_argument(
        "--compare", action="store_true", help="Compare current coverage to baseline"
    )
