...
from tests.performance.test_websocket_integrated_throughput import (
    TestServerManager,
    IntegratedWebSocketTester,
)
...
class WebSocketPerformanceSuite:
    ...
    async def run_mocked_performance_tests(self) -> Dict:
        ...
        reconnection_rate = (
            stab_metrics.reconnections_successful / max(stab_metrics.connection_dropouts, 1)
        ) * 100
    ...
    async def run_integration_performance_tests(self) -> Dict:
        ...
        try:
            ...
            logger.error(
                "aiohttp required for integration tests. Install with: pip install aiohttp"
            )
    ...
    def _generate_benchmark_analysis(self, results: Dict) -> Dict:
        ...
        if "mocked_tests" in results:
            ...
            if "multi_agent_conversation" in mocked:
                ...
                analysis["performance_summary"]["conversation_throughput_mps"] = conv.get(
                    "throughput_mps", 0
                )
                analysis["performance_summary"]["conversation_latency_p95_ms"] = conv.get(
                    "p95_latency_ms", 0
                )
                analysis["performance_summary"]["memory_per_connection_mb"] = conv.get(
                    "memory_per_connection_mb", 0
                )

            if "high_throughput_burst" in mocked:
                burst = mocked["high_throughput_burst"]
                analysis["performance_summary"]["peak_throughput_mps"] = burst.get(
                    "peak_throughput_mps", 0
                )
                analysis["performance_summary"]["burst_latency_p99_ms"] = burst.get(
                    "p99_latency_ms", 0
                )
            ...
            analysis["sla_compliance"]["avg_business_impact"] = total_business_impact / max(
                test_count, 1
            )
            ...
            if analysis["sla_compliance"]["avg_business_impact"] > 50:
                analysis["recommendations"].append(
                    "High business impact detected. Consider optimizing WebSocket performance."
                )

            if analysis["performance_summary"].get("conversation_latency_p95_ms", 0) > 200:
                analysis["recommendations"].append(
                    "P95 conversation latency exceeds 200ms. Optimize message processing."
                )

            if analysis["performance_summary"].get("memory_per_connection_mb", 0) > 34.5:
                analysis["recommendations"].append(
                    "Memory usage per connection exceeds 34.5MB budget. Investigate memory leaks."
                )

            if not analysis["recommendations"]:
                analysis["recommendations"].append(
                    "All performance metrics within acceptable ranges. System performing well."
                )
...
async def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["mocked", "integration", "benchmark"],
        default="mocked",
