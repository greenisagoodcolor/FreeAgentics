...
phase_2_repository_cleanup() {
    ...
    find . -name "test-reports" -type d -exec rm -rf {} + || true
    ...
    if command -v autoflake &> /dev/null; then
        ...
        autoflake --in-place --recursive --remove-all-unused-imports --remove-unused-variables . || true
    else
