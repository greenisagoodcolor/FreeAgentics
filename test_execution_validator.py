...
class TestExecutionValidator:
    ...
    def validate_test_collection(self) -> bool:
        ...
        returncode, stdout, stderr = self.run_pytest_command(["--collect-only", "-q"], timeout=60)
    ...
    def run_sample_tests(self, sample_patterns: List[str]) -> Dict[str, any]:
        ...
        for pattern in sample_patterns:
            ...
            returncode, stdout, stderr = self.run_pytest_command(
                [pattern, "-v", "--tb=no", "--no-header", "-q"], timeout=120
            )
    ...
    def run_comprehensive_validation(self) -> bool:
        ...
        print(
            f"📈 Target: ≥{self.target_success_rate}% success rate ({self.target_passes}/{self.total_collected} tests)"
        )
        print()
        ...
        if meets_target:
            ...
            print(
                f"   Achieved {success_rate:.2f}% success rate (≥{self.target_success_rate}% target)"
            )
        else:
