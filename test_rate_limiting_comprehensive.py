                ...
                try:
                    ...
                    if response.status_code == 403:  # Blocked for suspicious pattern
                        self.add_test_result(
                            ...
                            "passed",
                            {"message": f"Blocked after {i+1} 404 requests", "block_status": 403}
                        )
                        break
                ...
                try:
                    ...
                    if response.status_code == 403:
                        self.add_test_result(
                            ...
                            "passed",
                            {"message": f"Blocked after {i+1} path scans", "block_status": 403}
                        )
                        blocked = True
                        break
                except requests.RequestException:
            ...
            try:
                ...
                if response.status_code == 413:  # Request Entity Too Large
                    self.add_test_result(
                        test_name,
                        "passed",
                        {"message": "Large request properly rejected", "status_code": 413}
                    )
                else:
                    self.add_test_result(
                        test_name,
                        "failed",
                        {"message": "Large request not rejected", "status_code": response.status_code}
                    )
            except requests.RequestException as e:
                ...
                self.add_test_result(
                    test_name,
                    "passed",
                    {"message": "Large request blocked at connection level", "error": str(e)}
                )
