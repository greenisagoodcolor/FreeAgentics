    ...
    try:
        ...
        try:
            ...
            if result and not result.get("metadata", {}).get("error"):
                ...
                logger.info(
                    f"Update generated {len(entities)} entities, {len(relations)} relations, {len(update_events)} events"
                )

                # Check graph state
                graph_metrics = updater.get_metrics()
                logger.info(
                    f"Graph now has {graph_metrics['knowledge_graph_stats']['node_count']} nodes"
                )
    ...
    try:
        ...
        try:
            ...
            if result and not result.get("metadata", {}).get("error"):
                ...
                logger.info(
                    f"Updater metrics: {updater_metrics.get('successful_updates', 0)} successful updates"
                )
...
async def test_batch_processing():
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Batch processing: {updater_metrics.get('successful_updates', 0)} successful updates"
            )
...
async def test_schema_validation():
    ...
    try:
        from knowledge_graph.schema import ConversationOntology, SchemaValidator
        from knowledge_graph.pymdp_extractor import PyMDPKnowledgeExtractor
        ...
        logger.info(
            f"Validation: {valid_entities}/{len(result['entities'])} entities valid, "
