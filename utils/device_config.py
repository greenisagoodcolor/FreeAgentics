...
try:
    ...
    if not _FORCE_CPU:
        ...
        _CUDA_AVAILABLE = False
    logger.info(f"PyTorch device configuration: CUDA available={_CUDA_AVAILABLE}, Force CPU={_FORCE_CPU}")
except ImportError:
...
class DeviceConfig:
    ...
    def __init__(self, force_cpu: bool = False):
        ...
        if self.torch_available:
            self.device = torch.device("cpu" if self.force_cpu else ("cuda" if self.cuda_available else "cpu"))
        else:
            self.device = "cpu"
        
        logger.debug(f"DeviceConfig initialized: device={self.device}, torch_available={self.torch_available}")
...
class TensorAdapter:
    ...
    def to_tensor(self, array: Union[np.ndarray, list, float, int], dtype: Optional[Any] = None) -> Any:
...
def set_cpu_only_mode(enabled: bool = True) -> None:
    ...
    logger.info(f"Device configuration updated: CPU-only mode {'enabled' if enabled else 'disabled'}")