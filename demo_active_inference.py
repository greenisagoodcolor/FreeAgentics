...
try:
    ...
    logger.warning("PyMDP not available - using simplified Active Inference")
...
class GridWorld:
    ...
    def _initialize_world(self):
        ...
        logger.info(f"Created world: {self.width}x{self.height} with {len(self.resources)} resources, "
...
class ActiveInferenceAgent:
    ...
    def __init__(self, agent_id: str, name: str, position: Position, 
                 world: GridWorld, agent_type: str = "explorer"):
...
class ActiveInferenceDemo:
    ...
    def _create_agents(self, num_agents: int):
        ...
        names = [
            "Alpha Explorer", "Beta Collector", "Gamma Analyzer",
            "Delta Scout", "Epsilon Hunter", "Zeta Guardian"
        ]
    ...
    def _log_summary(self):
        ...
        avg_free_energy = np.mean([a.total_free_energy / max(1, a.steps_taken) 
                                   for a in self.agents])
        
        logger.info(f"Tick {self.tick}: Resources collected: {total_resources}, "
    ...
    def _init_visualization(self):
        ...
        for x in range(self.world.width):
            for y in range(self.world.height):
                ...
                if cell_type == CellType.WALL:
                    ...
                    circle = Circle((x, y), 0.4, color='lime', 
        ...
        self.agent_markers = {}
        for agent in self.agents:
            marker = Circle((agent.position.x, agent.position.y), 0.25,
                          color=agent.color, edgecolor='black', linewidth=2)
    ...
    def _update_energy_plot(self):
        ...
        for agent in self.agents:
            ...
            if history:
                self.ax_energy.plot(history[-50:], label=agent.name.split()[0],
                                  color=agent.color, linewidth=2)
